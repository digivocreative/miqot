...
const AGENTS: Record<string, { name: string; phone: string }> = {
...
function buildCard(p: Paket, phone: string): string {
...
.btn--sticky{padding:11px 22px;border-radius:var(--rs);font-size:13.5px;background:linear-gradient(135deg,#27AE60,#52C77E);color:var(--w);white-space:nowrap;box-shadow:0 2px 10px rgba(39,174,96,.22)}
...
export const onRequest = async (context: { params: { slug: string }; request: Request }) => {
  ...
  return new Response(generateHTML(slug), {
    status: 200,
