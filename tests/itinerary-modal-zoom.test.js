...
test('ItineraryModal supports two-finger pinch zoom for itinerary preview', () => {
  ...
  assert.match(source, /content\.style\.transform = `translate3d\(0, 0, 0\) scale\(\$\{nextScale\}\)`/);
  ...
  assert.match(source, /setViewerScale\(pinchRef\.current\.startScale \* \(dist \/ pinchRef\.current\.startDist\), center, \{ syncState: false \}\)/);
  ...
  assert.match(source, /touchAction:\s*'pan-x pan-y'[\s\S]*overscrollBehavior:\s*'contain'/);
