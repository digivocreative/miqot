...
function statusClasses(status: PortalPerlengkapanStateItem['status']) {
  if (status === 'diambil') return { box: 'bg-emerald-50 text-emerald-700', badge: 'bg-emerald-50 text-emerald-700' };
  if (status === 'tersedia') return { box: 'bg-amber-50 text-amber-700', badge: 'bg-amber-50 text-amber-700' };
  return { box: 'bg-slate-100 text-slate-400', badge: 'bg-slate-100 text-slate-500' };
}
...
export default function PerlengkapanItem({
  ...
  return (
    <div className={`rounded-2xl border bg-white p-4 shadow-sm ${item.status === 'tersedia' ? 'border-amber-200' : 'border-slate-100'}`}>
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-start justify-between gap-3">
            ...
            {item.status === 'diambil' ? (
              <span className="flex h-7 w-7 flex-none items-center justify-center rounded-full bg-emerald-50 text-emerald-700">
