import type { AgentData } from '../data/agents';

interface AgentProfileProps {
  agent: AgentData;
  packageName?: string;
  isCapturing?: boolean;
}

export default function AgentProfile({ agent, packageName = "Umrah Alhijaz", isCapturing = false }: AgentProfileProps) {
  if (!agent) return null;

  // Format link WA Dinamis
  const message = `Assalamualaikum, Saya mau tanya terkait paket ${packageName}`;
  const encodedMessage = encodeURIComponent(message);
  const waLink = `https://wa.me/${agent.phone}?text=${encodedMessage}`;

  // Format nomor WA untuk tampilan (628xxx → +62 8xxx)
  const formatPhone = (phone: string) => {
    const clean = phone.replace(/\D/g, '');
    if (clean.startsWith('62')) {
      return `+62 ${clean.slice(2)}`;
    }
    return phone;
  };

  return (
    // 1. Container dengan Gradient Background
    <div className="mt-2 mb-3 p-3 rounded-xl border border-emerald-100 dark:border-emerald-800/50 bg-gradient-to-r from-emerald-50 via-white to-white dark:from-emerald-950/40 dark:via-slate-800 dark:to-slate-800 flex items-center justify-between gap-3 shadow-sm animate-in fade-in slide-in-from-top-2">
      
      {/* KIRI: Foto & Info */}
      <div className="flex items-center gap-3 overflow-hidden">
        {/* Foto Profil dengan Ring */}
        <div className="relative w-11 h-11 flex-shrink-0">
            <img 
              src={agent.photo} 
              alt={agent.name} 
              className="w-full h-full object-cover rounded-full border-2 border-white dark:border-slate-700 shadow-md"
              onError={(e) => {
                  (e.target as HTMLImageElement).src = `https://ui-avatars.com/api/?name=${encodeURIComponent(agent.name)}&background=random`;
              }}
            />
        </div>

        {/* Teks Info */}
        <div className="flex flex-col min-w-0 justify-center">
          <h4 className="text-[13px] font-bold text-gray-900 dark:text-white truncate leading-tight mb-0.5">
            {agent.name}
          </h4>
          {/* Website Tanpa Icon */}
          <span className="text-[11px] text-gray-500 dark:text-slate-400 truncate font-medium">
            {agent.website}
          </span>
        </div>
      </div>

      {/* KANAN: Chat Button atau Nomor WA (saat capture) */}
      {isCapturing ? (
        <div className="flex-shrink-0 bg-emerald-500 text-white pl-3 pr-4 py-2 rounded-full flex items-center gap-1.5">
          <svg className="w-4 h-4 fill-white" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
            <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
          </svg>
          <span className="text-xs font-bold tracking-wide">{formatPhone(agent.phone)}</span>
        </div>
      ) : (
        <a 
          href={waLink}
          target="_blank"
          rel="noopener noreferrer"
          className="flex-shrink-0 bg-emerald-500 hover:bg-emerald-600 text-white pl-3 pr-4 py-2 rounded-full flex items-center gap-1.5 shadow-lg shadow-emerald-500/20 dark:shadow-emerald-500/10 transition-all active:scale-[0.96] group"
        >
          <svg className="w-4 h-4 fill-white group-hover:scale-110 transition-transform" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
            <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
          </svg>
          <span className="text-xs font-bold tracking-wide">Chat</span>
        </a>
      )}

    </div>
  );
}
