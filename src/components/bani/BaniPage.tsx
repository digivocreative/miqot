...
export default function BaniPage({ slug, onNavigate }: { slug: string; onNavigate: (path: string) => void }) {
  ...
  useEffect(() => {
    if (phase === 'answer') window.scrollTo({ top: 0, behavior: 'smooth' });
  }, [phase]);
  ...
  const openLinkTarget = useCallback((target: 'jamaah' | 'calendar' | 'jadwal') => {
    ...
    if (target === 'calendar') { onNavigate('/dashboard'); return; }
