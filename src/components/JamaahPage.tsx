...
export default function JamaahPage({ agentSlug, jamaahConnected, jamaahUser, initialSubTab = 'umroh', onConnectionChange, onHeaderRight, onNavigate }: JamaahPageProps) {
  ...
  // ── Targeted refresh: pull a single id_umroh (group) from API resmi after
  // adding a jamaah to an existing booking — much faster than a full sync. ──
  useEffect(() => {
    if (view !== 'data' || syncing) return;
    const params = new URLSearchParams(window.location.search);
    const refreshId = params.get('refresh_id_umroh');
    if (!refreshId) return;
                          ...
                          <div className="bg-gray-50/80 dark:bg-slate-900/40 px-3 py-2.5">
                            ...
                            <div className="relative">
                              <textarea
                                ...
                                maxLength={240}
                                autoFocus
                                // `coarse:leading-5`: font dipaksa 16px di perangkat sentuh (anti-zoom
                                // iOS) sementara line-height-nya rasio 1,5 — tiga baris tumbuh 18px→24px
                                // dan kotak catatan ini membengkak 72px→86px di tengah baris tabel.
                                // Dipatok 20px: cukup lega untuk 16px (tinta glif ±17px) dan tingginya
                                // kembali setara desain, 74px.
                              ...
                              />
                              {!noteText && (
                                // Baris pertama overlay harus jatuh persis di baris pertama textarea —
                                // ikut `coarse:leading-5`, kalau tidak bayangannya meleset 2px.
