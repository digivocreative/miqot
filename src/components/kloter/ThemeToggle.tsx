...
function readInitialTheme() {
  ...
  try {
    stored = window.localStorage.getItem(KLOTER_THEME_KEY);
  } catch {
    // Storage diblokir (mode privat): ikuti tema perangkat.
  }
...
export default function KloterThemeToggle() {
  ...
  return (
    <button
      ...
      className="touch-hit relative flex h-9 w-9 items-center justify-center rounded-lg bg-gray-100/80 dark:bg-slate-800/80 text-gray-500 transition-colors hover:bg-gray-200 active:scale-95 dark:text-slate-300 dark:hover:bg-slate-700"
    >
