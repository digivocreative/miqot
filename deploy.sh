...
# Seluruh badan deploy dibungkus main() dan baru dipanggil di baris terakhir.
# Alasannya: deploy.sh MENIMPA DIRINYA SENDIRI lewat `git pull` di tengah
# eksekusi. bash membaca skrip secara bertahap dari offset byte, jadi kalau
# file berubah panjang saat berjalan, offset lama menunjuk ke isi yang salah —
# bash bisa mengeksekusi ulang baris awal (terbukti: loop tak henti) atau
# menabrak potongan perintah. Dengan main(), bash WAJIB membaca & mem-parse
# seluruh file sebelum menjalankan apa pun, sehingga kebal terhadap perubahan
# on-disk. `exit` di baris akhir mencegah bash membaca sisa file yang tumbuh.
main() {
...
# `main` dan `exit` WAJIB satu compound command: bash harus membaca sampai `}`
# sebelum menjalankan isinya, jadi `exit` sudah ada di memori saat main selesai.
# Kalau ditulis dua baris terpisah, bash membaca baris kedua SETELAH main jalan —
# dan offset-nya sudah bergeser karena `git pull` mengubah panjang file, sehingga
# bash mengeksekusi potongan acak (terbukti di uji: "INJECTED: command not found").
{ main "$@"; exit "$?"; }
