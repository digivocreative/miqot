// Registry tool untuk permukaan AI yang membaca data Alhijaz: MCP eksternal
// per-agent (mcp-server.js, POST /mcp) — asisten milik agent sendiri
// (hermes/openclaw) memakai skema zod + envelope content MCP.
//
// Nama berkas masih "bani-" karena registry ini lahir sebagai lapisan bersama
// MCP + asisten in-app Bani. Bani sudah dihapus; nama dipertahankan supaya
// riwayat git dan guard source di tests/ tidak putus. Satu-satunya pemakai
// sekarang adalah mcp-server.js.
//
// Kontrak `run(deps, args)` sengaja NETRAL terhadap permukaan:
//   sukses → { ok: true,  data }
//   gagal  → { ok: false, error }
// Pemetaan ke bentuk khas permukaan (envelope content MCP) adalah urusan
// pemanggil, bukan handler.
//
...
import { TOUR_PHASES } from './journey-order.js';
import { canServeItineraryCache } from './itinerary-cache-freshness.js';
import {
  cityOnDate,
  isInCityOnDate,
  tourLabelsFromItinerary,
} from './bani-itinerary-index.js';

export const MAX_LIMIT = 50;
const BANI_ITINERARY_CANDIDATE_LIMIT = 60;
...
export const BANI_TOOLS = [
...

  {
    ...
    description: 'Daftar jadwal paket umroh Alhijaz (data global, bukan per-agent): tanggal berangkat/pulang, '
      ...
      + 'Filter keberangkatan lintas bulan dengan berangkat_from/berangkat_to; cari paket yang sedang berjalan '
      + 'pada tanggal tertentu dengan covers_date (contoh: paket yang sedang berjalan saat tahun baru → covers_date); '
      + 'dan coba beberapa ejaan nama paket sekaligus dengan search_any. '
      + 'Filter label tur dari itinerary tersimpan dengan tur, atau cek posisi Mekkah/Madinah pada tanggal tertentu '
      + 'dengan kota_pada_tanggal. Cache itinerary dibaca apa adanya tanpa memicu parsing PDF. '
    ...
    parameters: {
      ...
      properties: {
        ...
        berangkat_from: {
          type: 'string',
          pattern: ISO_DATE_PATTERN,
          description: 'Tanggal keberangkatan paling awal (YYYY-MM-DD, inklusif). Diabaikan bila month diisi.',
        },
        berangkat_to: {
          type: 'string',
          pattern: ISO_DATE_PATTERN,
          description: 'Tanggal keberangkatan paling akhir (YYYY-MM-DD, inklusif). Diabaikan bila month diisi.',
        },
        covers_date: {
          type: 'string',
          pattern: ISO_DATE_PATTERN,
          description: 'Paket yang sedang berjalan pada tanggal ini: sudah berangkat dan belum pulang (YYYY-MM-DD)',
        },
        ...
        search_any: {
          ...
          description: 'Cari nama paket yang cocok dengan salah satu istilah/ejaan (OR), maksimal 5',
        },
        tur: {
          type: 'string',
          enum: TOUR_PHASES.map((phase) => phase.label),
          description: 'Hanya paket yang itinerary tersimpannya memuat label tur ini',
        },
        kota_pada_tanggal: {
          type: 'object',
          properties: {
            tanggal: { type: 'string', pattern: ISO_DATE_PATTERN },
            kota: { type: 'string', enum: ['mekkah', 'madinah'] },
          },
          required: ['tanggal', 'kota'],
          additionalProperties: false,
          description: 'Filter paket berdasarkan posisi kota pada tanggal tertentu dari itinerary tersimpan',
        },
    ...
    run: async ({ supabase }, {
      ...
      // `month` sengaja menang dan rentang diabaikan tanpa error: model sering
      // mengirim keduanya untuk berjaga-jaga. covers_date tetap boleh digabung.
      if (!month && berangkat_from && !isRealISODate(berangkat_from)) return toolError('berangkat_from tidak valid — pakai format YYYY-MM-DD yang benar');
      if (!month && berangkat_to && !isRealISODate(berangkat_to)) return toolError('berangkat_to tidak valid — pakai format YYYY-MM-DD yang benar');
      if (covers_date && !isRealISODate(covers_date)) return toolError('covers_date tidak valid — pakai format YYYY-MM-DD yang benar');
      if (tur && !TOUR_PHASES.some((phase) => phase.label === tur)) return toolError('tur tidak valid — pilih label tur yang tersedia');
      if (kota_pada_tanggal) {
        if (!kota_pada_tanggal || typeof kota_pada_tanggal !== 'object' || Array.isArray(kota_pada_tanggal)) {
          return toolError('kota_pada_tanggal tidak valid — isi tanggal dan kota');
        }
        if (!isRealISODate(kota_pada_tanggal.tanggal)) {
          return toolError('Tanggal kota_pada_tanggal tidak valid — pakai format YYYY-MM-DD yang benar');
        }
        if (!['mekkah', 'madinah'].includes(kota_pada_tanggal.kota)) {
          return toolError('Kota tidak valid — pilih mekkah atau madinah');
        }
      }
      const today = jakartaToday();
      const FETCH_CAP = 500;
      let q = supabase
        ...
        .select('jadwal_id, jadwal_nama, promo, seat_total, seat_sisa, maskapai, berangkat_tgl, berangkat_jam, pulang_tgl, manasik_tgl, manasik_jam, paket_harga, synced_at, brosur, brosur_cdn, brosur_source_sha256, itinerary_cdn, itinerary_source_sha256')
      ...
      if (month) {
        ...
        const explicitRange = Boolean(berangkat_from || berangkat_to);
        if (berangkat_from) q = q.gte('berangkat_tgl', berangkat_from);
        if (berangkat_to) q = q.lte('berangkat_tgl', berangkat_to);
        // covers_date harus bisa menemukan paket yang sudah berangkat tetapi
        // belum pulang, jadi guard default upcoming tidak boleh ikut.
        if (!explicitRange && !covers_date && !include_departed) q = q.gte('berangkat_tgl', today);
      }
      if (covers_date) q = q.lte('berangkat_tgl', covers_date).gte('pulang_tgl', covers_date);

      const term = sanitizeSearchTerm(search);
      if (term) q = q.ilike('jadwal_nama', `*${term}*`);
      const anyTerms = (Array.isArray(search_any) ? search_any : [])
        .slice(0, 5)
        .filter((item) => typeof item === 'string')
        .map((item) => sanitizeSearchTerm(item).slice(0, 40))
        .filter((item, index, items) => item && items.indexOf(item) === index);
      if (anyTerms.length) {
        q = q.or(anyTerms.map((item) => `jadwal_nama.ilike.*${item}*`).join(','));
      }
      ...
      const scheduleById = new Map(deduped.map(row => [String(row.jadwal_id), row]));
      ...
      // Cache itinerary adalah pengayaan sekunder dan dibaca HANYA untuk hasil
      // kandidat query jadwal ini. Urutan query sudah keberangkatan terdekat,
      // sehingga pemotongan tetap deterministik dan tidak memilih paket acak.
      const itineraryCandidatesLimited = rows.length > BANI_ITINERARY_CANDIDATE_LIMIT;
      rows = rows.slice(0, BANI_ITINERARY_CANDIDATE_LIMIT);
      const candidateIds = rows.map((row) => row.jadwal_id).filter(Boolean);
      let itineraryById = new Map();
      if (candidateIds.length) {
        const { data: itineraryRows, error: itineraryError } = await supabase
          .from('itineraries')
          .select('jadwal_id, content, source_sha256')
          .in('jadwal_id', candidateIds);
        if (itineraryError) throw new Error(itineraryError.message);
        itineraryById = new Map(
          (itineraryRows || [])
            .filter((row) => row?.jadwal_id)
            .filter((row) => canServeItineraryCache(row, scheduleById.get(String(row.jadwal_id))))
            .map((row) => [String(row.jadwal_id), row.content]),
        );
      }

      let uncertainCityCount = 0;
      const enrichedRows = [];
      for (const row of rows) {
        const itineraryKey = String(row.jadwal_id ?? '');
        const hasCachedItinerary = itineraryById.has(itineraryKey);
        const content = hasCachedItinerary ? itineraryById.get(itineraryKey) : null;
        const tourLabels = tourLabelsFromItinerary(content);
        if (tur && !tourLabels.includes(tur)) continue;

        let itineraryAvailable = hasCachedItinerary && content != null;
        const enriched = {
          ...row,
          tur: tourLabels,
          itinerary_tersedia: itineraryAvailable,
        };
        if (kota_pada_tanggal) {
          const { tanggal, kota } = kota_pada_tanggal;
          const cityMatch = isInCityOnDate(content, row.berangkat_tgl, tanggal, kota);
          if (cityMatch === false) continue;
          if (cityMatch === null) itineraryAvailable = false;
          enriched.itinerary_tersedia = itineraryAvailable;
          enriched.kota_pada_tanggal = cityOnDate(content, row.berangkat_tgl, tanggal);
          if (!itineraryAvailable) uncertainCityCount += 1;
        }
      ...
      const rangeIgnored = Boolean(month && (berangkat_from || berangkat_to));
      const notes = [];
      if (rangeIgnored) notes.push('Filter berangkat_from/berangkat_to diabaikan karena month menang.');
      if (itineraryCandidatesLimited) {
        notes.push(`Penyaringan itinerary dibatasi ke ${BANI_ITINERARY_CANDIDATE_LIMIT} keberangkatan terdekat.`);
      }
      if (uncertainCityCount) {
        notes.push(`${uncertainCityCount} paket belum punya itinerary tersimpan, posisi kotanya belum bisa dipastikan.`);
      }
      notes.push(GLOBAL_NOTE);
      return toolResult({
        total: rows.length,
        page: Math.max(1, Number(page) || 1),
        limit: cappedLimit,
