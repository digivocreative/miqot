...
/**
 * @deprecated Not used by the live read path — {@link computeDailyReadPlan}
 * (via fetchEventsForRange) is the canonical range-split routine. Kept only for
 * its existing unit tests; do not build new code on this. Prefer computeDailyReadPlan.
 *
  ...
  try {
    ...
    while (cursorKey <= yesterdayKey) {
      ...
      try {
        ...
        // Isolate per-day failures: one poisoned day must not abort the whole cycle
        // and thereby skip cleanup (the prod incident — a single FK-violating day
        // wedged retention for weeks). Log it and move on; it retries next run.
        failedDays++;
        console.error(`[Analytics] Aggregation failed for ${cursorKey}, skipping that day:`, err.message);
      }
      cursorKey = nextKey;
    }
    if (failedDays > 0) {
      console.warn(`[Analytics] ${failedDays} day(s) failed to aggregate this cycle; cleanup still ran.`);
    }
    if (aggResult.days === 0) {
    ...
    // Only reached if we couldn't even read the aggregate state (infra error).
    // Per-day aggregation failures are isolated inside the loop above.
    console.error('[Analytics] Aggregation setup failed, skipping cleanup:', err.message);
