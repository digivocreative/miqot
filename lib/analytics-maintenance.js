...
export async function runAnalyticsMaintenance(supabase) {
  ...
  // Align DELETE cutoff to UTC midnight — matches computeRangeSplit's boundary,
  // so raw retention is contiguous with the agg range queried by fetchEventsForRange.
 ...
 *
 * Assumes the daily aggregation cron has run successfully through yesterday.
 * If the cron has missed days that are now past the 14d cutoff, those events
 * will have been deleted from raw without ever being aggregated — straddle-month
 * queries will undercount for the affected date range. Remedy: run the backfill
 * script (scripts/backfill-analytics-daily.js) to retroactively aggregate any
 * days present in raw but missing from the aggregate table.
 *
