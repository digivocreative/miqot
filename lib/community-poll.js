 ...
 * teks, durasi dipilih dari daftar tetap (default 24 jam). Suara satu per
 * agent, boleh diganti selama polling terbuka, tidak bisa dicabut.
 */
...
/**
 * Durasi yang bisa dipilih (key dikirim klien di `poll.duration`). Angka bebas
 * sengaja TIDAK diterima — daftar tertutup membuat validasi & UI sederhana.
 */
export const COMMUNITY_POLL_DURATIONS = Object.freeze({
  '1d': 24 * 60 * 60 * 1000,
  '3d': 3 * 24 * 60 * 60 * 1000,
  '7d': 7 * 24 * 60 * 60 * 1000,
});
export const COMMUNITY_POLL_DEFAULT_DURATION = '1d';

function fail(error) {
  return { options: null, durationMs: null, error };
}

/**
 * Validasi input poll dari klien (`body.poll`). Menerima `{ options: [...],
 * duration? }`, mengembalikan `{ options: string[], durationMs: number,
 * error: null }` atau `{ options: null, durationMs: null, error: string }`.
 * Teks di-trim; kosong/duplikat/kepanjangan/durasi di luar daftar ditolak
 * dengan pesan yang bisa langsung dipakai sebagai respons 400. `duration`
 * absen jatuh ke default 24 jam (klien lama tetap jalan).
 */
export function normalizeCommunityPollInput(raw) {
  if (raw === undefined || raw === null) return { options: null, durationMs: null, error: null };
  ...
  const durationKey = raw.duration === undefined ? COMMUNITY_POLL_DEFAULT_DURATION : raw.duration;
  const durationMs = COMMUNITY_POLL_DURATIONS[durationKey];
  if (!durationMs) return fail('Durasi polling tidak valid');
  ...
  return { options, durationMs, error: null };
}
