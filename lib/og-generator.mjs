 ...
 * agent. Bedanya SENGAJA lapang di tengah — tanpa chip, panel harga, atau blok
 ...
 *
 * `eyebrow` & `headline` datang dari lib/filter-share-meta.js; jangan menyusun
 * teksnya di sini, kalau tidak judul tab dan kartu WhatsApp bisa menyebut filter
...
export async function generateFilterOgPng({ eyebrow, headline, agentName, agentPhotoBuffer } = {}) {
  ...
  // Turun satu tingkat ukuran dulu sebelum membiarkan teksnya terpotong — pola
  // sama dengan nama paket panjang di generatePackageOgPng.
  const TITLE_STEPS = [
    { size: 72, step: 82, maxLines: 2, eyebrowGap: 74 },
    { size: 56, step: 64, maxLines: 3, eyebrowGap: 60 },
  ];
  const fitted = TITLE_STEPS
    .map(s => ({ ...s, lines: wrapOgLines(rawHeadline, s.size, 700, s.maxLines) }))
    .find(s => !s.lines.some(line => line.endsWith('…')))
    || { ...TITLE_STEPS[TITLE_STEPS.length - 1], lines: wrapOgLines(rawHeadline, 56, 700, 3) };

  const titleLines = fitted.lines.map(escapeXml);
  const lastBaseline = 372;
  const firstBaseline = lastBaseline - fitted.step * (titleLines.length - 1);
  const eyebrowBaseline = firstBaseline - fitted.eyebrowGap;

  const safeEyebrow = escapeXml(
    stripUnrenderableGlyphs(String(eyebrow || 'JADWAL UMROH')).toUpperCase()
  );
  const safeAgent = escapeXml(truncateOgText(stripUnrenderableGlyphs(agentName) || '', 28));
  const safeInitials = escapeXml(ogInitials(agentName));
  ...
  const svg = Buffer.from(`
    <svg width="${W}" height="${H}" viewBox="0 0 ${W} ${H}" xmlns="http://www.w3.org/2000/svg">
      ...
      <circle cx="1160" cy="50" r="260" fill="#D4AF37" opacity="0.08"/>
      <circle cx="70" cy="660" r="230" fill="#34D399" opacity="0.08"/>
      <circle cx="1000" cy="260" r="178" fill="url(#fltHalo)"/>
      ...
      <rect x="${MARGIN}" y="420" width="96" height="5" rx="2.5" fill="#D4AF37"/>
      <text x="${MARGIN}" y="466" font-family="Inter, Arial, sans-serif" font-size="22" font-weight="600" fill="#FFFFFFCC">Lihat jadwal, harga, dan sisa kursi</text>
  ...
  // Watermark Ka'bah lebih besar & lebih ke bawah daripada kartu paket: kanan
  // bawah kartu ini kosong (tanpa blok hotel), jadi ia justru mengisi ruang itu.
  const emblemPath = path.join(PROJECT_ROOT, 'public', 'img-brosur', 'kabah.png');
  if (fs.existsSync(emblemPath)) {
    try {
      const scaled = await sharp(emblemPath).resize({ width: 360 }).ensureAlpha().png().toBuffer();
      const { width: ew, height: eh } = await sharp(scaled).metadata();
      const emblem = await sharp(scaled)
        .composite([{ input: Buffer.from(`<svg width="${ew}" height="${eh}"><rect width="${ew}" height="${eh}" fill="#ffffff4a"/></svg>`), blend: 'dest-in' }])
        .png()
        .toBuffer();
      composites.push({ input: emblem, left: 810, top: 150 });
    } catch (err) {
