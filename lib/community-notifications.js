...
/**
 * A reply that @mentions someone produces one comment row and one mention row
 * for the same event (comment_id = that comment). Drop the comment side when a
 * mention already represents it — the mention is more specific and wins.
 */
function dedupeCommentsAgainstMentions(mentions, comments) {
  const mentionedCommentIds = new Set(
    (mentions || []).map(row => row?.comment_id).filter(Boolean)
  );
  return (comments || []).filter(row => !mentionedCommentIds.has(row.id));
}
...
/**
 * @param {{mentions?: object[], comments?: object[], reactions?: object[], broadcasts?: object[]}} sources
...
export function mergeNotifications(sources, seenAt, limit = NOTIFICATION_LIMIT) {
  ...
  for (const row of dedupeCommentsAgainstMentions(sources?.mentions, sources?.comments)) {
    items.push({
      id: `comment:${row.id}`,
      type: 'comment',
      post_id: row.post_id,
 ...
 * and comments after dropping the ones already represented by a mention (see
 * dedupeCommentsAgainstMentions), so a mentioning reply isn't counted twice.
 */
export function countUnreadNotifications(sources) {
  const total = (sources?.mentions || []).length
    + dedupeCommentsAgainstMentions(sources?.mentions, sources?.comments).length
