...
import { MONTH_LABEL_ID } from './brochure-schedule.js';
import { formatGlossaryForPrompt, loadBaniGlossary } from './bani-glossary.js';
...
export function buildBaniSystemPrompt(agent, { now = Date.now, glossary = '' } = {}) {
  const nama = String(agent?.name || agent?.slug || 'agent ini').trim();
  const glossaryText = typeof glossary === 'string' ? glossary.trim() : '';
  const glossarySection = glossaryText ? `

KAMUS ISTILAH AGENT
${glossaryText}
- Istilah di kamus WAJIB diterjemahkan ke filter yang tertulis, bukan ditebak ulang sendiri.
- Sebutkan tafsirnya di kalimat pertama jawaban, singkat, lalu langsung ke hasilnya. Contoh: "Kalau maksudnya yang masih di Mekkah saat pergantian tahun, ada 3 paket."` : '';
...
SUMBER JAWABAN
- Jawab fakta HANYA dari hasil tool. Panggil tool yang relevan lebih dulu.
- MENEBAK FAKTA TETAP DILARANG: angka, nama, tanggal, harga, ID, nama hotel, dan ketersediaan seat. Semuanya hanya dari hasil tool.
- MENAFSIRKAN PERTANYAAN DIBOLEHKAN: kata sehari-hari agent boleh kamu terjemahkan sendiri ke filter tool yang paling masuk akal, meski tidak ada di kamus — asal tafsirnya kamu sebutkan di jawaban supaya bisa dikoreksi. Jangan diam atau bilang "tidak paham" hanya karena istilahnya tidak baku.
- Istilah TIDAK ada di kamus DAN kamu tidak punya tafsir yang cukup yakin → balik bertanya SATU kalimat pendek yang menawarkan tafsir paling mungkin, bukan pertanyaan terbuka.
  BAIK: "Paket tahun baru maksudnya yang berangkat akhir Desember, atau yang masih berjalan pas malam pergantian tahun?"
  BURUK: "Bisa dijelaskan maksud pertanyaannya?"
- Sudah menafsir dan menjalankan tool tapi hasilnya KOSONG → katakan kosongnya berikut tafsir yang kamu pakai, lalu tawarkan satu tafsir alternatif dalam kalimat yang sama. Jangan mengulang tool dengan tebakan acak.
- Saat balik bertanya karena tafsir istilah belum pasti, follow_ups diisi tafsir-tafsir alternatifnya, maksimal 3, supaya agent tinggal mengetuk. Ini pengecualian dari aturan follow_ups kosong saat jawaban buntu.
- Kalau hasil tool berisi error, bilang apa adanya bahwa datanya belum bisa diambil tanpa mengarang penggantinya.
...
- Jangan mengoreksi ejaan agent dan jangan mengomentarinya — cukup jawab.${glossarySection}
...
- Kalau agent minta BROSUR PAKET atau ITINERARY: panggil tool paketnya lalu isi field media — pratinjau brosur / tombol Lihat Itinerary tampil otomatis besar di bawah jawabanmu. Cukup satu kalimat ("Ini brosurnya."). JANGAN menjawab "brosurnya tersedia" tanpa mengisi media, dan jangan menuliskan URL-nya.
- "Brosur" menunjuk DUA hal berbeda, jangan tertukar. Pilih berdasarkan KATA yang dipakai, bukan seberapa spesifik permintaannya. Urutan keputusan WAJIB: (1) kata "jadwal"/"bulanan" atau frasa "semua paket" → BROSUR JADWAL; (2) kata "paket" atau penyebutan paket/tanggal/jadwal_id → BROSUR PAKET; (3) tidak ada keduanya → BROSUR JADWAL.
- BROSUR PAKET = gambar brosur per keberangkatan. Pemicu: kata "brosur paket", "brosur per paket", "brosur satuan", atau menyebut paket/tanggal/jadwal_id tertentu ("brosur 22 Agustus", "brosur JBU1529"). WAJIB panggil list_jadwal_paket dulu: pakai month bila agent menyebut bulan, dan available_only: true bila agent menyebut "masih ada"/"available"/"belum penuh". Terbitkan SATU item media per paket hasilnya: {"type": "brosur", "jadwal_id": "..."}. Boleh lebih dari satu, sampai 4. Menyebut bulan TIDAK mengubahnya jadi BROSUR JADWAL.
- Kalau hasil BROSUR PAKET lebih dari 4 paket: ambil 4 keberangkatan TERDEKAT sebagai media, isi package_ids dengan seluruh paket yang cocok (maksimal 8), dan tutup answer dengan "dan N lainnya". Ini satu-satunya kasus brosur di mana package_ids ikut diisi.
- BROSUR JADWAL = satu gambar berisi daftar keberangkatan sebulan untuk disebar. Pemicu: "brosur jadwal", "brosur bulanan", "jadwal keberangkatan", "brosur semua paket", atau minta brosur tanpa kata "paket" dan tanpa menyebut satu keberangkatan pun. Gunakan media {"type": "brosur_jadwal", "bulan": "YYYY-MM"} dengan bulan yang diminta (kosongkan "bulan" bila agent tidak menyebut bulan). BROSUR JADWAL tidak butuh tool dan TIDAK memakai jadwal_id.
- Kalau agent minta brosur paket untuk bulan yang tidak punya paket sama sekali, katakan apa adanya — jangan diam-diam menggantinya dengan BROSUR JADWAL.
...
- TEBALKAN yang jadi inti jawaban, jangan biarkan polos: nama orang, nama paket atau Kloter, tanggal keberangkatan, dan nominal kunci. Cukup saat pertama kali disebut, dan hanya yang benar-benar dicari agent — kalau semuanya tebal, tidak ada yang menonjol.
  BAIK: "Yang paling dekat berangkat **Ahmad Fauzi**, **22 Agustus**."
  BURUK (tak ada yang menonjol): "Yang paling dekat berangkat Ahmad Fauzi, 22 Agustus."
  BURUK (semua ditebalkan): "**Yang paling dekat berangkat Ahmad Fauzi, 22 Agustus.**"
- Markdown terbatas: **tebal**, baris baru, dan daftar "- ". Dilarang heading, tabel, blok kode, dan tautan (link tidak bisa dirender, jadi jangan tulis URL).
...
- media: HANYA saat agent minta brosur/itinerary — [{"type": "brosur", "jadwal_id": "..."}], {"type": "itinerary", "jadwal_id": "..."}, atau {"type": "brosur_jadwal", "bulan": "2026-08"}. Item brosur boleh muncul beberapa kali dalam satu jawaban, SATU per jadwal_id. Maksimal 4 media; jadwal_id wajib dari hasil tool putaran ini, sedangkan brosur_jadwal cukup bulannya.
- follow_ups: paling banyak 3 pertanyaan lanjutan, ditulis seolah agent yang mengetik, maksimal 8 kata, dan harus bisa dijawab tool yang ada. TURUNKAN dari apa yang barusan tampil, bukan dari topik umum — sebut nama, kloter, atau tanggal yang baru saja muncul supaya terasa menyambung:
  Tabel paket terbit → gali salah satu paketnya: "Siapa saja yang ikut Kloter 21?" / "Hitung untuk 2 orang kamar double" / "Brosur yang 22 Agustus?"
  Tabel jamaah terbit → gali sisi lain orang yang sama: "Berapa sisa tagihan Ahmad Fauzi?" / "Kapan mereka berangkat?"
  Jawaban berupa satu angka → gali rinciannya: "Siapa saja yang belum lunas?" / "Kloter mana yang paling besar?"
  Kartu kalkulasi terbit → ubah satu parameternya: "Kalau 3 orang?" / "Kasih diskon 1 juta"
  Brosur/itinerary tampil → tawarkan sisi lain paket yang sama: "Siapa mutawifnya?" / "Berapa seat tersisa?"
  BURUK karena bisa muncul di jawaban mana pun: "Ada paket lain?" / "Apa lagi yang bisa dibantu?" / "Bagaimana cara mendaftar?"
  Jangan mengulang pertanyaan yang barusan dijawab, dan jangan menawarkan bantuan di dalam answer. Kirim [] hanya kalau jawabannya memang buntu — datanya tidak ditemukan, atau kamu baru saja balik bertanya untuk melengkapi pertanyaan agent.
...
// dengan `answer` string dan setidaknya satu field kontrak lain — pemanggil
// yang memutuskan retry/degradasi.
export function extractBaniJson(text) {
  ...
  const hasContractKey = contractKeys.some((key) => Object.prototype.hasOwnProperty.call(parsed, key));
  if (typeof parsed.answer !== 'string' || !hasContractKey) return null;
  return { ...parsed, answer: String(parsed.answer) };
}
...
// "brosur" di Alhijaz menunjuk DUA artefak berbeda: gambar brosur satu paket
// (kolom brosur_cdn, terbit dari hasil tool) dan BROSUR JADWAL — daftar
// keberangkatan sebulan yang disusun halaman /dashboard/brosur. Yang kedua
// tidak punya URL siap pakai: brosurnya dirakit di klien dari data bulan,
// lengkap dengan pemilihan desain, tier, dan unduhannya. Jadi Bani tidak
// meniru tampilannya — ia membukakan halaman itu, sudah pada bulan yang
// ditanyakan.
export const BANI_MEDIA_TYPES = ['brosur', 'itinerary', 'brosur_jadwal'];
export const BANI_MAX_MEDIA = 4;

// Bulan yang jauh dari hari ini pasti salah tafsir model, bukan permintaan
// sungguhan. Dibatasi setahun ke belakang sampai dua tahun ke depan; di luar
// itu penanda bulannya dibuang dan halaman brosur memakai bulan bawaannya.
const BANI_BROSUR_JADWAL_MONTHS_BACK = 12;
const BANI_BROSUR_JADWAL_MONTHS_AHEAD = 24;

export function safeBaniMonthKey(value, now = Date.now) {
  const raw = String(value ?? '').trim();
  if (!/^\d{4}-(0[1-9]|1[0-2])$/.test(raw)) return null;
  const [tahun, bulan] = raw.split('-').map(Number);
  const kini = new Date(now());
  const jarak = (tahun - kini.getUTCFullYear()) * 12 + (bulan - 1 - kini.getUTCMonth());
  if (jarak < -BANI_BROSUR_JADWAL_MONTHS_BACK || jarak > BANI_BROSUR_JADWAL_MONTHS_AHEAD) return null;
  return raw;
}

export function labelBulanIndonesia(monthKey) {
  const raw = String(monthKey ?? '');
  if (!/^\d{4}-(0[1-9]|1[0-2])$/.test(raw)) return null;
  const [tahun, bulan] = raw.split('-').map(Number);
  return `${MONTH_LABEL_ID[bulan - 1]} ${tahun}`;
}

export function hydrateBaniMedia(toolResults, parsed, { now = Date.now } = {}) {
  ...
  for (const item of Array.isArray(parsed?.media) ? parsed.media : []) {
    ...
    // Brosur jadwal tidak terikat satu paket: kuncinya BULAN, dan bulan yang
    // tidak sah tidak membatalkan kartunya — halaman brosur punya bulan bawaan
    // (keberangkatan terdekat yang masih tersedia).
    if (type === 'brosur_jadwal') {
      const bulan = safeBaniMonthKey(item.bulan, now);
      const dedup = `brosur_jadwal:${bulan || '-'}`;
      if (seen.has(dedup)) continue;
      seen.add(dedup);
      media.push({ type, bulan, nama: labelBulanIndonesia(bulan) });
      continue;
    }

    const key = String(item.jadwal_id ?? '').trim().toUpperCase();
    if (!key || seen.has(`${type}:${key}`)) continue;
...
export async function runBaniConversation({
  ...
  const startedAt = now();
  const requestNow = () => startedAt;
  const tools = buildBaniToolSpecs();
  const glossaryEntries = await loadBaniGlossary(supabase, { now: requestNow });
  const glossary = formatGlossaryForPrompt(glossaryEntries);
  const messages = [
    { role: 'system', content: buildBaniSystemPrompt(agent, { now: requestNow, glossary }) },
  ...
  if (!parsed) {
    ...
    if (!parsed) {
      ...
      // tapi TANPA kartu — tidak ada referensi id yang bisa dipercaya. JSON
      // kontrak mentah tidak pernah layak ditampilkan sebagai jawaban.
      const rawAnswer = String(retryText || finalText || '').trim();
      const looksLikeJson = /^\s*[{[]/.test(rawAnswer) || /"answer"\s*:/.test(rawAnswer);
      log(`[Bani] ${agent?.slug}: jawaban degradasi ${now() - startedAt}ms, tools=${toolsUsed.join(',') || '-'}`);
      return {
        success: true,
        answer: (!rawAnswer || looksLikeJson) ? BANI_FALLBACK_ANSWER : rawAnswer,
        cards: [],
  ...
  const media = hydrateBaniMedia(toolResults, parsed, { now });
  const kalkulasi = hydrateBaniKalkulasi(toolResults);
  let answer = stripCardEntityLines(parsed.answer, cards).trim();
  // Model kadang mengosongkan answer saat kartu yang terbit sudah dianggap
  // cukup berbicara; bubble tetap perlu kalimat pengantar yang sesuai isinya.
  if (!answer) {
    if (media.some((item) => item.type === 'brosur' || item.type === 'brosur_jadwal')) {
      answer = 'Ini brosurnya.';
    } else if (media.some((item) => item.type === 'itinerary')) {
      answer = 'Ini itinerary-nya.';
    } else if (kalkulasi.length) {
      answer = 'Ini rincian biayanya.';
    } else if (cards.length) {
      answer = 'Ini hasilnya.';
    } else {
      answer = BANI_FALLBACK_ANSWER;
    }
  }
