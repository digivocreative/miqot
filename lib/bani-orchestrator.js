...
export const BANI_JAMAAH_COLUMNS = ['berangkat', 'sisa', 'bayar', 'paket', 'kode'];
...
function sanitizeShownRef(item) {
  ...
  const type = item.type === 'package' || item.type === 'jamaah' ? item.type : null;
  const id = typeof item.id === 'string' ? item.id.replace(/[^\w-]/g, '').slice(0, 30) : '';
  if (!type || !id) return null;
  const nama = typeof item.nama === 'string'
...
function formatShownNote(shown) {
  const parts = shown.map((s) => `${s.type === 'package' ? 'paket' : 'jamaah'} ${s.id}${s.nama ? ` "${s.nama}"` : ''}`);
...
- Kalau agent minta HITUNGAN BIAYA ("berapa untuk 2 orang kamar double", "hitung sekeluarga 4 pax"): panggil kalkulasi_harga — JANGAN menghitung sendiri dari harga paket. Hasilnya otomatis tampil sebagai kartu rincian + total dengan tombol salin teks WA dan PDF, jadi answer cukup 1–2 kalimat; total akhirnya boleh disebut, rincian per barisnya jangan diulang. Kosongkan package_ids kecuali daftar paketnya memang ditanya juga. Kalau jumlah orang/kamarnya belum jelas, tanyakan dulu dalam satu kalimat singkat.
...
- jamaah_columns: kolom tabel jamaah, MAKSIMAL 2, pilih yang menjawab pertanyaannya. Yang sah: "berangkat", "sisa", "bayar", "paket", "kode". Ditanya keberangkatan → ["berangkat"]; ditanya pelunasan/tunggakan → ["sisa"]; ditanya setoran → ["bayar"]. JANGAN memasang "sisa" kalau yang ditanya bukan soal uang.
