...
export const BANI_SOURCE_NOTE = 'Snapshot hasil sync, bukan real-time';
...
export function buildBaniSystemPrompt(agent) {
  const nama = String(agent?.name || agent?.slug || 'agent ini').trim();
  return `Kamu Bani, asisten agent di Miqot — dashboard kerja agent umroh & haji Alhijaz.
...
export async function runBaniConversation({
  ...
  const messages = [
    { role: 'system', content: buildBaniSystemPrompt(agent) },
