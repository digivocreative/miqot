...
/**
 * Canonical departure instant in epoch milliseconds.
 * AirLabs timestamps are seconds; calendar/itinerary `_depUTC` values are ms.
 */
export function departureTimestampMs(entry) {
  const explicit = positiveFiniteNumber(entry?.departureTimestamp);
  if (explicit !== null) return explicit;

  const providerSeconds = positiveFiniteNumber(entry?.depTs);
  if (providerSeconds !== null) return providerSeconds * 1000;

  return positiveFiniteNumber(entry?._depUTC);
}
...
function localDepartureKey(entry) {
  const date = dateKey(entry?.eventDate)
    || dateKey(entry?.depDate)
    || dateKey(entry?.depScheduled);
  const time = timeKey(entry?.depActual) || timeKey(entry?.depScheduled);
  return `${date}T${time}`;
}

/**
 * Ascending chronological order. Local clock text is only a fallback when
 * neither entry has a trustworthy absolute timestamp.
 */
export function compareFlightDepartureTimestamp(left, right) {
  const leftTimestamp = departureTimestampMs(left);
  const rightTimestamp = departureTimestampMs(right);
  if (leftTimestamp !== null && rightTimestamp !== null) {
    return leftTimestamp - rightTimestamp;
  }
  return localDepartureKey(left).localeCompare(localDepartureKey(right));
}

function publicEntry(entry) {
  const departureTimestamp = departureTimestampMs(entry);
  const {
    _mergeSourceKey,
    _mergeCardKey,
    _mergeTlKey,
  ...
  return { ...rest, departureTimestamp };
}
