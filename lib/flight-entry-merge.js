...
function transitLabelForSegments(entries) {
  ...
  for (let i = 0; i < entries.length - 1; i += 1) {
    ...
    const city = cleanText(current.arrCity || next.depCity || current.arrCode || next.depCode);
    ...
    const minutes = Number.isFinite(arrUTC) && Number.isFinite(nextDepUTC) && nextDepUTC > arrUTC
      ...
      : null;
    if (city || minutes) stops.push({ city, minutes });
  }

  if (stops.length === 0) return null;
  if (stops.length > 1) {
    const cities = uniqueOrdered(stops.map(stop => stop.city));
    return cities.length > 0 ? `Transit ${stops.length}x: ${cities.join(', ')}` : `Transit ${stops.length}x`;
  }
