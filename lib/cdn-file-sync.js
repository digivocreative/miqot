 ...
 *
 * Huruf kapital yang membedakan: slug nama paket selalu huruf kecil, sedangkan
 * token acak hampir selalu bercampur kapital. Kalau sesekali meleset, ruginya
 * cuma satu siklus reset yang sembuh sendiri di sync berikutnya.
 */
function stripVolatileScheduleToken(pathname) {
  if (!/^\/(brosur|itinerary)\//i.test(pathname)) return pathname;
  return pathname.replace(/-(?=[A-Za-z0-9]{6,12}$)(?=[A-Za-z0-9]*[A-Z])[A-Za-z0-9]+$/, '');
}
