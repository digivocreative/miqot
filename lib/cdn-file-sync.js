 ...
 *
 * Yang membedakan token dari kata: panjang 5–16 dan memuat angka ATAU huruf
 * kapital. Slug nama paket lahir dari judul berbahasa Indonesia — huruf kecil
 * semua, dan potongan berangkanya pendek (`9hr`, `12hr`). Diuji atas 120 URL
 * brosur yang benar-benar tersimpan (19 Sep 2026): tokennya terpotong semua.
 *
 * Kedua arah kesalahannya TIDAK setara, dan aturannya sengaja condong ke satu
 * sisi. Memotong yang seharusnya tinggal cuma membuat satu penggantian brosur
 * telat ketahuan sampai sync berikutnya — sha256 byte sumber tetap hakimnya.
 * Sebaliknya, GAGAL memotong berarti paket itu kehilangan salinan CDN-nya tiap
 * 30 menit. Syarat "wajib ada kapital" pernah dicoba dan gagal begitu: 7 paket
 * ter-reset tiap siklus karena tokennya kebetulan huruf kecil semua (`hrh6gfr`).
 */
function stripVolatileScheduleToken(pathname) {
  if (!/^\/(brosur|itinerary)\//i.test(pathname)) return pathname;
  return pathname.replace(/-(?=[A-Za-z0-9]{5,16}$)(?=[A-Za-z0-9]*[A-Z0-9])[A-Za-z0-9]+$/, '');
}
