...
export function normalizeBunnyDownloadUrl(url) {
  ...
  try {
    ...
    // Host publik yang membungkus foto ikut terkena blokir Cloudflare untuk
    // request server. Lewati hanya host resmi ini menuju origin yang sama agar
    // foto partner baru tetap dapat dimirror ke Bunny dan disajikan via HTTPS.
    if (hostname === 'alhijazindowisata.com' || hostname === 'www.alhijazindowisata.com') {
      return new URL(`${parsed.pathname}${parsed.search}`, 'http://115.124.86.220').toString();
    }

    // Origin jadwal hanya membuka port 80; menaikkannya ke HTTPS akan selalu
    // gagal koneksi. URL HTTP lain mempertahankan perilaku lama (upgrade HTTPS).
    if (hostname === '115.124.86.220') return parsed.toString();
