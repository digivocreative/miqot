...
// A named venue (café/lounge/hotel/…) plus its name. The name is often the
// gathering spot but written mid-sentence, e.g. "…makan siang di café Zukavia",
// which cleanLocation would otherwise strip as a trailing activity.
const VENUE_RE = /\b(?:caf[eé]|lounge|hotel|resto|restaurant|lobby)\s+[A-Za-z][\w’'.]*(?:\s+\d+)?/i;

// Keep the named venue even when it sits after a trailing-activity phrase that
// cleanLocation drops. If the cleaned base already contains it (venue stated up
// front), leave it in place; otherwise surface it at the front.
function withRescuedVenue(raw) {
  const base = normalizeLocationLabels(raw);
  const venue = raw.match(VENUE_RE)?.[0]?.replace(/\s+/g, ' ').replace(/[.,;:]+$/, '').trim() || '';
...
function meetingLocation(text) {
  ...
  return raw ? withRescuedVenue(raw) : null;
}
