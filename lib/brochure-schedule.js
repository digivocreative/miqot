...
/** Paket WAITING LIST adalah placeholder internal untuk menampung peminat,
 * bukan keberangkatan yang boleh dipromosikan di brosur/katalog. */
export function isWaitingListPackageName(name) {
  const normalized = String(name || '')
    .toUpperCase()
    .replace(/[^A-Z0-9]+/g, ' ')
    .trim();
  return /\bWAITING\s*LIST\b/.test(normalized);
}
