...
export function serializeScheduleRows(rows = [], journeyOrderById = new Map()) {
  return (rows || []).map(row => {
    ...
    // Turunan kecil (lebar 400px, ~30 KB) yang dipakai kartu & modal sebagai
    // bayangan kabur selagi brosur penuhnya diunduh. Kolom _cdn di bawah tetap
    // dibuang: yang keluar hanya URL yang sudah lolos cek fingerprint, jadi
    // thumb dari versi brosur lama tidak mungkin ikut tersaji.
    out.brosur_thumb = resolveScheduleBrochureThumbUrl(out);
