...
export function serializeScheduleRows(rows = [], journeyOrderById = new Map(), options = {}) {
  const preferSourceUrls = options?.preferSourceUrls === true;
  ...
  return (rows || []).map(row => {
    ...
    if (out.brosur_cdn && (!preferSourceUrls || !out.brosur)) out.brosur = out.brosur_cdn;
    if (out.itinerary_cdn && (!preferSourceUrls || !out.itinerary)) out.itinerary = out.itinerary_cdn;
