...
function avatarFallback(name) {
  return `https://ui-avatars.com/api/?name=${encodeURIComponent(name || 'Agent')}&background=random&size=192`;
}
...
function buildKursHtml({ kurs, agent, baseUrl }) {
  ...
  const website = agent.website && agent.website.trim()
    ...
    : `wa.me/${wa}`;
  const logoSrc = `${baseUrl}/logo-alhijaz-besar.svg`;
...
export async function generateKursImageBuffer({ kurs, agent, format = 'jpeg', quality = 88 }) {
  const baseUrl = process.env.NOTIFIER_BASE_URL || 'http://localhost:3000';
  const html = buildKursHtml({ kurs, agent, baseUrl });
