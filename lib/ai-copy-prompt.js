...
export function formatTanggalIndonesia(value) {
  ...
  if (year < 1 || day < 1 || day > daysInMonth) return raw;
  return `${day} ${BULAN_INDONESIA[month - 1]} ${year}`;
}

function normalizeTanggalCaption(text) {
  return String(text)
    // ISO dari model, mis. 2026-08-28.
    .replace(/\b(\d{4})-(\d{2})-(\d{2})\b/g, (value) => formatTanggalIndonesia(value))
    // Format angka umum dari model, mis. 28/08/2026 atau 28-08-2026.
    .replace(/\b(\d{1,2})[/-](\d{1,2})[/-](\d{4})\b/g, (value, day, month, year) => {
      const iso = `${year}-${String(month).padStart(2, '0')}-${String(day).padStart(2, '0')}`;
      const formatted = formatTanggalIndonesia(iso);
      return formatted === iso ? value : formatted;
    });
}
...
- Semua tanggal WAJIB ditulis dengan nama bulan Indonesia lengkap, contoh: *28 Agustus 2026*. JANGAN gunakan format ISO (2026-08-28), format angka (28/08/2026), atau singkatan bulan (28 Agu 2026).
...
- Jangan gunakan tag HTML atau HTML entity seperti &nbsp;, &#160;, dan &#xA0;. Gunakan spasi biasa dan baris baru.
...
Tanggal Berangkat: ${formatTanggalIndonesia(pkg.keberangkatan?.tgl)}
Tanggal Pulang: ${formatTanggalIndonesia(pkg.kepulangan?.tgl)}
...
function buildMonthPrompts(monthData, agentName, agentWebsite) {
  const rows = monthData.packages.slice(0, MAX_MONTH_ROWS).map((p) => {
    const parts = [
      formatTanggalIndonesia(p.berangkat_tgl),
...
function toWaFormat(text) {
  return normalizeTanggalCaption(text)
    // Output model kadang memakai HTML entity/non-breaking space untuk merapikan
    // jarak. Caption dirender dan disalin sebagai plain text, jadi normalkan ke
    // spasi biasa agar entity tidak tampil literal di preview maupun WhatsApp.
    .replace(/&(?:amp;)?(?:nbsp|#0*160|#x0*a0);?/gi, ' ')
    .replace(/\u00a0/g, ' ')
