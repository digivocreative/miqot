 ...
 *     is BERANGKAT (raw legacy scrape value). Even if status_bayar=LUNAS, stage 2 stays belumCair
 ...
 *   CICILAN/LUNAS/LEBIH BAYAR + BERANGKAT → full rate cair
 */
...
export function computeBreakdownTahun(rows) {
  ...
  for (const r of rows) {
    ...
    if (b === 'BERANGKAT') entry.sudahBerangkat++;

    const hasPaid = s === 'CICILAN' || s === 'LUNAS' || s === 'LEBIH BAYAR';
    if (hasPaid) {
      entry.komisiCair += KOMISI_STAGE1;
      if (b === 'BERANGKAT') entry.komisiCair += stage2;
    }
  }
...
/**
 * Count jamaah by departure state. BERANGKAT = departed; else (BELUM,
 * empty, null, etc.) = not yet departed.
 */
export function computeBerangkatStats(rows) {
  let sudah = 0, belum = 0;
  for (const r of rows) {
    if (norm(r.status_berangkat) === 'BERANGKAT') sudah++;
    else belum++;
  }
