...
export const PACKAGE_VALUE_PROMPT_VERSION = '2026-07-17-v19-routine-transport-clean';
...
function findEvidenceBindingIssue(text, evidenceFact, factGroups) {
  ...
  if (DISPLAY_HARD_FORBIDDEN_PATTERN.test(text)) return 'internal-copy';
...
function buildEvidenceCatalog(packageData, itinerary) {
  ...
  add('B03', 'brosur', [packageData.airline, packageData.departure.flight].filter(Boolean).length
    ? `Maskapai/penerbangan berangkat: ${[packageData.airline, packageData.departure.flight, directFact].filter(Boolean).join(' • ')}`
    : '');
