...
export const PACKAGE_VALUE_PROMPT_VERSION = '2026-07-17-v17-semantic-binding-relation-safe';
...
const SOURCE_REQUIRED_CLAIM_GROUPS = [
  ...
  { id: 'claim:proximity', terms: ['dekat', 'strategis', 'jarak'] },
  { id: 'claim:luxury', terms: ['mewah', 'kemewahan'] },
  { id: 'claim:guarantee', terms: ['menjamin', 'terjamin', 'terpercaya', 'pasti'] },
  { id: 'claim:superlative', terms: ['terbaik', 'termurah', 'terlengkap', 'maksimal', 'eksklusif'] },
  { id: 'claim:direct-flight', terms: ['tanpa transit', 'penerbangan langsung', 'direct flight'] },
...
const DISPLAY_FORBIDDEN_PATTERN = /\b(?:tier|uhud|rahmah|hemat|vip|quard|quad|double|triple|single)\b/i;
...
function buildCatalogFactGroups(evidenceCatalog) {
  ...
  for (const evidence of Array.isArray(evidenceCatalog) ? evidenceCatalog : []) {
    ...
    for (const match of matches) {
      if (!CATALOG_NAMED_TERM_STOP_WORDS.has(match) && !/^\d+$/.test(match)) {
        namedTerms.add(match.toLocaleLowerCase('id-ID'));
      }
    }
  }
  return [
...
function findEvidenceBindingIssue(text, evidenceFact, factGroups) {
  ...
  if (DISPLAY_FORBIDDEN_PATTERN.test(text)) return 'internal-copy';
  ...
  const evidenceNumbers = new Set(groundingNumbers(evidenceFact));
...
function sentenceFromEvidence(value) {
  const cleanValue = cleanText(value, 150)
    .replace(/[.;,\s]+$/, '')
    .split(/\s+/)
    .slice(0, 12)
    .join(' ');
...
function buildEvidenceBoundAdvantage(evidence) {
  ...
  if (hotelMatch) {
    ...
    const shortHotel = fullHotel.split(/\s+/).slice(0, 3).join(' ');
    return {
      title: `Hotel ${shortHotel}`,
      benefit: `Menjalani hari ${city} dari ${shortHotel}`,
      description: `Menginap di ${fullHotel} selama berada di ${city}.`,
    };
  }

  if (/^Maskapai\/penerbangan|^Maskapai\b/i.test(fact)) {
    const airline = displayFactText((fact.split(':').slice(1).join(':').split('•')[0] || '').trim());
    return {
  ...
  if (/\b(?:kereta|haramain)\b/i.test(fact)) {
    ...
    return {
      ...
      benefit: provesTrainRide
        ...
        : 'Perpindahan itinerary mencakup stasiun kereta cepat',
  ...
  if (/\b(?:pesawat|penerbangan|airlines?|maskapai)\b/i.test(fact)) {
    const destination = places.at(-1);
    const operatorMatch = payload.match(/pesawat\s+(.+?)(?:\s+(?:menuju|dari|jamaah|jemaah|kembali|tiba|melanjutkan|dan)\b|$)/i);
  ...
  if (/\b(?:city\s*tour|citytour|tour|mengunjungi)\b/i.test(fact) && (places.length || landmarks.length)) {
    ...
    return {
      title: `Jelajah ${place}`,
      benefit: highlights ? `Menjelajahi ${highlights} dalam agenda ${place}` : `Mengikuti agenda perjalanan di ${place}`,
      description: sentenceFromEvidence(payload),
    };
  }

  if (/\bHarga\b/i.test(fact)) {
    const price = formatCompactMillionPrice(fact);
    return {
      title: `Harga ${price}`,
  ...
  return {
    title,
    benefit: `Mengikuti agenda ${title}`,
    description: sentenceFromEvidence(payload),
  };
}
...
function buildEvidenceCatalog(packageData, itinerary) {
  ...
  const fallbacks = [
    ['B04', packageData.departure.route ? `Rute berangkat: ${packageData.departure.route}` : ''],
    ['B06', packageData.departure.date ? `Tanggal berangkat: ${packageData.departure.date}` : ''],
    ['B09', priceFacts.length ? `Harga tier ${packageData.tier || '-'} — ${priceFacts.join(', ')}` : ''],
  ];
...
export function parsePackageValueResult(content, {
  ...
  const mappedAdvantages = (Array.isArray(parsed?.advantages) ? parsed.advantages : [])
    .map((item) => {
      ...
      let title = cleanText(item?.title, 80);
      let benefit = cleanText(item?.benefit, 160);
      let description = cleanText(item?.description, 420);
      ...
      if (allowEvidenceRewrite && evidence && !unavailableItinerary && (!complete || bindingIssue || !anchoredTitle)) {
        ...
        title = cleanText(fallback.title, 80);
        benefit = cleanText(fallback.benefit, 160);
        description = cleanText(fallback.description, 420);
  ...
  let headline = cleanText(parsed?.headline, 140) || 'Nilai Plus Paket';
  let summary = cleanText(parsed?.summary, 600);
  let visualIdea = cleanText(parsed?.visualIdea, 220);
  if (enforceEvidenceIds) {
    ...
    if (headlineIssue || summaryIssue || visualIssue) return null;
  }
