...
export const MAX_THREAD_SEGMENTS = 10;
...
export function normalizeThreadSegments(payload) {
  ...
  for (let i = 0; i < segments.length; i += 1) {
    ...
    // Server hanya menghormati @semua di segmen pertama (lihat server.js),
    // tapi mengetik @semua dengan tangan di segmen lain harus ditolak di sini
    // juga — kalau tidak, composer menampilkan pil broadcast yang menjanjikan
    // sesuatu yang diam-diam tidak pernah terjadi saat diterbitkan.
    if (i > 0 && hasEveryoneMention(segment.body)) {
      return fail('@semua hanya boleh di kiriman pertama');
    }
  }
...
export function collectThreadMentions(segments, memberSlugs, authorSlug, limit) {
  const seen = new Set();
  const out = [];
  // Materialize memberSlugs into a Set once before the loop to avoid exhausting
  // single-use iterators (e.g. Map.prototype.keys()). toSlugSet fastpaths on Set.
  const allowedMembers = memberSlugs instanceof Set ? memberSlugs : new Set(memberSlugs || []);
  for (const segment of segments) {
    const slugs = extractCommunityMentions(segment.body, allowedMembers, authorSlug, limit);
