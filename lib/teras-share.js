...
/** Longest excerpt a crawler-visible preview may show of a post body. */
export const TERAS_PREVIEW_EXCERPT_LEN = 120;

/**
 * Teaser excerpt for link previews. The share link is login-gated but its
 * preview is not, so this deliberately stays short: enough to recognise the
 * post, never the whole thing. Newlines collapse into spaces so the excerpt
 * stays a single line in both the meta description and the OG card.
 */
export function terasPreviewExcerpt(body, maxLength = TERAS_PREVIEW_EXCERPT_LEN) {
