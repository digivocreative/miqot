...
/**
 * A marker-only calendar event reports the leg that is operational on the
 * event date. Keep that leg, while preserving its position in the full
 * schedule so route lookup still resolves the matching route pair.
 */
export function selectCalendarReportedSegments(pesawat, { eventType, schedule } = {}) {
  const scheduleSegments = parseFlightSegmentsFromCalendar(pesawat, { eventType, schedule });
  const calendarSegments = parseFlightSegmentsFromCalendar(pesawat, {
    eventType,
    schedule: null,
  });

  if (calendarSegments.length === 0) return [];

  const scheduleIndexByFlight = new Map(
    scheduleSegments.map((segment, index) => [segment.flightIata, index]),
  );
  return calendarSegments.flatMap(segment => {
    const segmentIndex = scheduleIndexByFlight.get(segment.flightIata);
