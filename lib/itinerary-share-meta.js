 ...
 */
/**
 * Kecilkan seluruh token lalu besarkan HURUF pertamanya — bukan karakter
 * pertamanya. Token yang diawali tanda baca ("(KERETA") kalau tidak akan
 * berakhir sebagai "(kereta": yang dikapitalkan cuma kurung bukanya.
 */
function capitalizeFirstLetter(token) {
  return token.toLowerCase().replace(/[a-z]/, (c) => c.toUpperCase());
}
...
export function formatPackageTitle(raw) {
  ...
  return text
    ...
    .map((token, i) => {
      ...
      // Tanda baca ikut menempel di token ("(KERETA", "CEPAT)"), jadi aturan
      // stopword & akronim dinilai dari HURUFNYA saja — kalau tidak, "(SV)"
      // tak pernah dikenali sebagai kode.
      const letters = token.replace(/[^A-Za-z]/g, '');
      // Dicek sebelum aturan akronim, kalau tidak "BY" akan lolos sebagai
      // kode dua huruf dan tak pernah dikecilkan.
      if (TITLE_STOPWORDS.has(letters)) {
        return i > 0 ? token.toLowerCase() : capitalizeFirstLetter(token);
      }
      if (/^[A-Z]{1,3}$/.test(letters)) return token;
