...
// - Mina, asisten AI in-app di dashboard (function calling) — memakai
...
// Pemetaan ke bentuk khas permukaan (content MCP, atau pesan tool Mina) adalah
// urusan pemanggil, bukan handler.
//
// READ-ONLY by design: modul ini tidak boleh memanggil .insert/.update/
// .upsert/.delete/.rpc — dijaga source grep di tests/mina-tools.test.js
...
// Hasil netral — pemanggil (mcp-server.js / endpoint Mina) yang membungkusnya
...

  {
    ...
    parameters: {
      ...
      properties: {
        ...
        search: { type: 'string', maxLength: 80, description: 'Cari di nama paket / nama Tour Leader / nomor grup' },
      },
      required: [],
    },
...
// Lookup by name — mcp-server.js (dan nanti endpoint Mina) mengambil entri
// tanpa menyisir array tiap request.
export const MINA_TOOL_BY_NAME = Object.fromEntries(MINA_TOOLS.map((tool) => [tool.name, tool]));
