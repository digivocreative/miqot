...
/**
 * Sub-nilai filter "AWAL PERJALANAN", urutan tampilnya. Nilainya = `tone`
 * simpul pertama rantai Urutan Perjalanan di kartu (src/utils/journey.ts)
 * dalam huruf besar. Roster TERTUTUP — ia juga menutup pola slug `awal-*`.
 */
export const JOURNEY_START_FILTER_VALUES = ['UMROH', 'MADINAH', 'TOUR'];

const JOURNEY_START_LABEL = { UMROH: 'Umroh', MADINAH: 'Madinah', TOUR: 'Tour' };
...
/** 'MADINAH' → 'awal-madinah' */
function journeyStartSlug(value) {
  const key = String(value || '').trim().toUpperCase();
  return JOURNEY_START_FILTER_VALUES.includes(key)
    ? `${JOURNEY_START_SLUG_PREFIX}${key.toLowerCase()}`
    : null;
}

function journeyStartFromSlug(slug) {
  if (!slug.startsWith(JOURNEY_START_SLUG_PREFIX)) return null;
  const key = slug.slice(JOURNEY_START_SLUG_PREFIX.length).toUpperCase();
  return JOURNEY_START_FILTER_VALUES.includes(key) ? key : null;
}
