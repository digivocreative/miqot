...
// `.` / `?` / `:` are ambiguous — each is as common as a mid-URL character
// ("...a.html", "...a?q=1", "...a:8080") as it is ordinary sentence
// punctuation right after a URL ("lihat https://x.id/a?", "link https://x.id/a: keren",
// "akhir https://x.id/a..."). A RUN of these ambiguous characters only counts
// as "still part of the URL" when a genuine URL-continuation character
// immediately follows the whole run; a run that ends the string, or is
// followed by whitespace/other punctuation (e.g. the `!` in "https://x.id/a?!"),
// does not block removal.
const AMBIGUOUS_CONTINUATION_CHARS = new Set(['.', '?', ':']);

/**
 * True when `body[pos]` continues a URL, so an occurrence ending at `pos`
 * must NOT be treated as a standalone, removable token. See
 * AMBIGUOUS_CONTINUATION_CHARS above for the `.`/`?`/`:` run-lookahead rule.
 */
function continuesUrl(body, pos) {
  if (pos >= body.length) return false;
  let i = pos;
  while (i < body.length && AMBIGUOUS_CONTINUATION_CHARS.has(body[i])) i += 1;
  if (i === pos) {
    // First character isn't ambiguous — decide directly: unambiguous
    // URL-continuation characters always continue the URL, anything else
    // (whitespace, `!`, etc.) never does.
    return URL_CONTINUATION_RE.test(body[pos]);
  }
  // Consumed a run of ambiguous characters: the run belongs to the URL only
  // if a genuine URL-continuation character immediately follows it.
  return i < body.length && URL_CONTINUATION_RE.test(body[i]);
}
