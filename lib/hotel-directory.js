...
// Dipertahankan (kosong = tanpa whitelist) demi kompatibilitas mundur bila ada pemanggil lain.
export const HOTEL_DIRECTORY_AGENT_SLUGS = new Set();

export const HOTEL_CITIES = ['mekkah', 'madinah', 'turki', 'dubai', 'kairo', 'haikou'];
...
// FAQ dibatasi 5 butir (permintaan user) — cermin CHECK hotel_faq_is_valid di
// migrations/20260816040000_hotel_faq.sql. Ubah di satu tempat = ubah di dua.
export const HOTEL_MAX_FAQ_ITEMS = 5;
...
export function isHotelDirectoryEnabledForAgent(_agentOrSlug) {
  return true;
}
...
// Galeri hotel VERSI AGENT SENDIRI (fitur terpisah dari media resmi di atas).
// Satu baris per (hotel, agent) di tabel hotel_agent_media — lihat migrations/
// 20260830010000_hotel_agent_media.sql.
export const HOTEL_AGENT_MEDIA_MAX_ITEMS = 12;
export const HOTEL_AGENT_MEDIA_NOTE_MAX = 300;

// Mirip normalizeHotelMediaInput, dengan dua beda sengaja:
// (1) tanpa kategori — galeri pribadi tidak perlu pengelompokan kurasi;
// (2) WAJIB kepemilikan: nama file harus diawali `{agentSlug}-` (pola nama
// file dari endpoint upload, cermin normalizeCommunityMediaInput di
// server.js) supaya seorang agent tidak bisa mengklaim URL unggahan agent
// lain sebagai "foto versi saya".
export function normalizeHotelAgentMediaInput(value, publicUrlPrefixes = [], agentSlug = '') {
  if (!Array.isArray(value) || value.length === 0 || value.length > HOTEL_AGENT_MEDIA_MAX_ITEMS) return null;
  ...
  for (const item of value) {
    ...
    const hasExpectedExtension = item.type === 'image'
      ...
      : /\.(?:mp4|mov)$/.test(path);
    if (!hasExpectedExtension || seenUrls.has(item.url)) return null;
    const fileName = candidate.pathname.slice(candidate.pathname.lastIndexOf('/') + 1);
    if (!fileName.toLowerCase().startsWith(`${slug}-`)) return null;
    seenUrls.add(item.url);
...
// null = ditolak (pemanggil membalas 400); {media, note} = siap simpan.
export function buildHotelAgentMediaPayload(input, { agentSlug = '', mediaPrefixes = [] } = {}) {
  if (!input || typeof input !== 'object') {
    return { ok: false, error: 'Payload galeri tidak valid' };
  }
  const media = normalizeHotelAgentMediaInput(input.media, mediaPrefixes, agentSlug);
  if (!media) {
    return {
      ok: false,
      error: `Daftar foto/video tidak valid (1-${HOTEL_AGENT_MEDIA_MAX_ITEMS} item, wajib hasil unggahan Anda sendiri)`,
    };
  }
  const noteResult = cleanText(input.note, HOTEL_AGENT_MEDIA_NOTE_MAX);
  if (!noteResult.ok) {
    return { ok: false, error: `Catatan maksimal ${HOTEL_AGENT_MEDIA_NOTE_MAX} karakter` };
  }
  return { ok: true, data: { media, note: noteResult.value } };
}
...
export function buildHotelPayload(input, { mediaPrefixes = [] } = {}) {
  ...
  const faqResult = normalizeHotelFaqInput(input.faq);
  if (!faqResult.ok) return { ok: false, error: faqResult.error };
...
// Butir FAQ kosong DIBUANG diam-diam (baris kosong di form editor bukan galat),
// tetapi butir yang hanya terisi separuh ditolak — pertanyaan tanpa jawaban
// tampil sebagai accordion buntu di halaman detail.
export function normalizeHotelFaqInput(value) {
  if (value === undefined || value === null) return { ok: true, value: [] };
  if (!Array.isArray(value)) return { ok: false, error: 'FAQ harus berupa daftar tanya-jawab' };

  const faq = [];
  for (const item of value) {
    if (!item || typeof item !== 'object' || Array.isArray(item)) {
      return { ok: false, error: 'FAQ harus berupa daftar tanya-jawab' };
    }
    if ((item.q !== undefined && typeof item.q !== 'string')
      || (item.a !== undefined && typeof item.a !== 'string')) {
      return { ok: false, error: 'Pertanyaan dan jawaban FAQ harus berupa teks' };
    }
    const q = (item.q || '').trim();
    const a = (item.a || '').trim();
    if (!q && !a) continue;
    if (!q || !a) return { ok: false, error: 'Setiap FAQ wajib punya pertanyaan dan jawaban' };
    if (q.length > LIMITS.faq_question) {
      return { ok: false, error: `Pertanyaan FAQ maksimal ${LIMITS.faq_question} karakter` };
    }
    if (a.length > LIMITS.faq_answer) {
      return { ok: false, error: `Jawaban FAQ maksimal ${LIMITS.faq_answer} karakter` };
    }
    faq.push({ q, a });
  }
  if (faq.length > HOTEL_MAX_FAQ_ITEMS) {
    return { ok: false, error: `FAQ maksimal ${HOTEL_MAX_FAQ_ITEMS} pertanyaan` };
  }
