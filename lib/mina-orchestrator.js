// Mina — orchestrator function calling untuk asisten AI in-app agent.
//
// Alur: pertanyaan agent → model memilih tool dari MINA_TOOLS (lib/mina-tools.js,
...
// hydrateMinaCards mengisi kartu dari row hasil tool request ini; id yang tidak
// pernah muncul di hasil tool dibuang diam-diam.
//
// READ-ONLY: modul ini tidak boleh menulis ke database sama sekali — dijaga
// source grep di tests/mina-orchestrator.test.js.
//
...
import { MINA_TOOLS, MINA_TOOL_BY_NAME } from './mina-tools.js';
...
export const MINA_MAX_ROUNDS = 3;
// Plafon eksekusi tool per request — melindungi DB (IO-sensitif) dari model
// yang menembak tool bertubi-tubi.
export const MINA_MAX_TOOL_CALLS = 6;
// Hemat token: tool daftar dipangkas 20 baris walau MAX_LIMIT registry 50.
export const MINA_TOOL_ROW_LIMIT = 20;
export const MINA_MAX_CARDS_PER_TYPE = 4;
export const MINA_SOURCE_NOTE = 'Snapshot hasil sync, bukan real-time';
const MINA_LINK_TARGETS = new Set(['jamaah', 'calendar', 'jadwal']);
const MINA_FALLBACK_ANSWER = 'Maaf, jawabannya belum bisa dirangkum. Coba tanya ulang dengan lebih spesifik.';

export function buildMinaSystemPrompt(agent) {
  const nama = String(agent?.name || agent?.slug || 'agent ini').trim();
  return `Kamu Mina, asisten agent di Miqot — dashboard kerja agent umroh & haji Alhijaz.
...
- Dilarang mengarang angka, nama, tanggal, harga, atau ID. Kalau hasil tool kosong atau berisi error, katakan datanya tidak ditemukan lalu sarankan langkah lanjutan yang masuk akal.
- Data adalah snapshot hasil sync dari sistem Alhijaz, bukan real-time. Sebut hal ini singkat bila jawabannya menyangkut nominal pembayaran.
- Jangan menampilkan nomor paspor atau data pribadi yang tidak ditanyakan.

GAYA
- Bahasa Indonesia, sapa pembacanya dengan "Anda". Hangat, langsung ke inti.
...
- Markdown terbatas: **tebal** untuk angka/nama/tanggal penting, baris baru, dan daftar dengan "- ". Dilarang heading, tabel, dan blok kode.
...
export function buildMinaToolSpecs() {
  return MINA_TOOLS.map((tool) => ({
    type: 'function',
    function: {
      name: tool.name,
...
export function extractMinaJson(text) {
...
export function hydrateMinaCards(toolResults, parsed) {
  ...
  const collect = (ids, index, build) => {
    ...
    for (const id of Array.isArray(ids) ? ids : []) {
      if (seen.size >= MINA_MAX_CARDS_PER_TYPE) break;
  ...
  if (MINA_LINK_TARGETS.has(link)) cards.push({ type: 'link', target: link });
...
async function executeMinaToolCall({ call, agent, supabase, log, budgetLeft }) {
  const name = call?.function?.name || '';
  const fail = (error) => ({ name, counted: false, record: null, content: JSON.stringify({ error }) });

  const tool = MINA_TOOL_BY_NAME[name];
  ...
  // Hemat token: paginasi tool daftar dipangkas ke MINA_TOOL_ROW_LIMIT.
  if (tool.parameters?.properties?.limit) {
    const asked = Number(args.limit);
    args.limit = Number.isFinite(asked) && asked > 0 ? Math.min(asked, MINA_TOOL_ROW_LIMIT) : MINA_TOOL_ROW_LIMIT;
  }
  ...
  log(`[Mina] ${agent?.slug}: ${name} (${Object.keys(args).join(',') || 'no args'})`);

  try {
    const out = await tool.run({ supabase, agent, log }, args);
    if (out?.ok) {
      return { name, counted: true, record: { name, ok: true, data: out.data }, content: JSON.stringify(out.data) };
    }
    return {
      name,
      counted: true,
    ...
    log(`[Mina] ${agent?.slug}: ${name} ERROR ${err.message}`);
    return {
      name,
      counted: true,
      record: { name, ok: false, data: null },
...
export async function runMinaConversation({
  question,
  agent,
  supabase,
  log = () => {},
  ...
  if (typeof callOpenAI !== 'function') throw new Error('runMinaConversation: callOpenAI wajib diinjeksi');
  const startedAt = now();
  const tools = buildMinaToolSpecs();
  const messages = [
    { role: 'system', content: buildMinaSystemPrompt(agent) },
  ...
  for (let round = 0; round < MINA_MAX_ROUNDS; round += 1) {
    ...
    for (const call of calls) {
      const outcome = await executeMinaToolCall({
        call,
        agent,
        supabase,
        log,
        budgetLeft: MINA_MAX_TOOL_CALLS - toolCallCount,
      });
      if (outcome.counted) {
        toolCallCount += 1;
  ...
  let parsed = extractMinaJson(finalText);
  if (!parsed) {
    ...
    parsed = extractMinaJson(retryText);
    ...
    if (!parsed) {
      ...
      log(`[Mina] ${agent?.slug}: jawaban degradasi ${now() - startedAt}ms, tools=${toolsUsed.join(',') || '-'}`);
      return {
        success: true,
        answer: rawAnswer || MINA_FALLBACK_ANSWER,
        cards: [],
  ...
  const cards = hydrateMinaCards(toolResults, parsed);
  log(`[Mina] ${agent?.slug}: jawaban siap ${now() - startedAt}ms, tools=${toolsUsed.join(',') || '-'}, cards=${cards.length}`);
  return {
    success: true,
