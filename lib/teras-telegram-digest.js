  ...
  for (const row of rows || []) {
    ...
    if (!existing) {
      groups.set(key, {
        ...
        // Kutipan balasan/aksi TERBARU — dipakai komentar untuk memberi konteks
        // isi, bukan sekadar hitungan. Reaksi tidak membawanya (null).
    ...
    if (toTime(row.created_at) > toTime(existing.created_at)) {
      ...
      existing.snippet = row.snippet || existing.snippet;
    }
  }
...
export function buildTerasDigestMessages({ comments, reactions, mentions, owners, origin }) {
  ...
  for (const group of commentGroups) {
    ...
    // Kutipan balasan terbaru sebagai blockquote yang bisa dikembangkan —
    // memberi isi/konteks supaya "3 balasan baru" tak lagi hambar.
    const quote = group.snippet
      ? `\n\n<blockquote expandable>${escapeHtml(group.snippet)}</blockquote>`
      : '';
    messages.push({
      ...
      type: 'comment',
      text: `💬 ${label}${quote}`,
      // Tautan dipisah dari teks: pemanggil merakitnya jadi tombol inline
      // (reply_markup), bukan URL telanjang di badan pesan.
      url: postUrl(group.post_id),
