...
export function collapseBookingOutstanding(rows) {
  ...
  for (const b of byBooking.values()) {
    ...
    if (b.aggregateShape) {
      // Owing rows shape aggregate replikasi: proven>0 menang, selain itu max
      // sisa (perilaku konservatif lama). Netting kredit TIDAK dipakai di sini —
      // shape aggregate sudah ambigu, biarkan jalur lama yang konservatif.
      const proven = provenAggregateOutstanding(b.owing);
      outstanding = Number.isFinite(proven) && proven > 0 ? proven : b.maxSisa;
    } else {
