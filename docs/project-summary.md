...
Tarik-untuk-segarkan (pull to refresh) hidup HANYA di app terpasang (`display-mode: standalone` / `navigator.standalone`) dengan pointer kasar — di tab browser, Chrome/Safari sudah punya gesturnya sendiri. Mesinnya `src/components/pwa/PullToRefresh.tsx` (indikator + listener) di atas `src/lib/pwa/pull-to-refresh.js` (ambang, redaman, kelayakan titik mulai). Gesturnya selalu memanggil pemuatan ulang data halaman, BUKAN `location.reload()`. Selama gestur aktif, `<html>` memakai kelas `pull-refresh-host` (`overscroll-behavior-y: contain`) supaya PTR bawaan Chrome Android tidak ikut menyala dan rubber-band iOS tidak berkelahi dengan indikator.

- **Jadwal publik** (tahap 1, Sep 2026): `src/App.tsx` memasang `<PullToRefresh>` langsung; penyegarnya revalidasi paket yang sama dengan interval 30 menit.
- **Dashboard & Teras** (tahap 2, Sep 2026): shell memasang `<PullToRefreshHost>`, dan HALAMAN yang sedang tampil mendaftarkan penyegarnya lewat `usePullRefreshHandler` ke tumpukan di `src/lib/pwa/refresh-registry.js` (pendaftar terakhir menang, giliran kembali otomatis saat ia lepas). Yang sudah terdaftar: Home (`refreshAgent` + `loadKurs` + remount widget lewat `homeRefreshKey`), feed & profil Teras (`refreshFeed`), dan Daftar Jamaah (bump `jamaahRefreshKey`, jalur muat-ulang yang memang sudah dipakai saat kembali dari sub-halaman).
- **Sisa halaman data** (tahap 3, Sep 2026): Statistik (`fetchStats` tahun terpilih, mati selama gerbang PIN), Analytics (`fetchData` bulan+tahun terpilih), Kelola Agent (`fetchAgents`), Direktori Hotel (penghitung `refreshTick` masuk ke dependensi efek muatnya — daftar, banner, dan detail yang sedang dibuka), serta detail utas Teras (`detailFetchTick`; jalur yang sama dengan tombol "Coba lagi"-nya). Yang dipilih SELALU pemuatan ulang data, bukan remount, supaya pilihan tahun/bulan/filter di halaman itu tidak ikut kembali ke bawaan.
- Halaman yang TIDAK mendaftar sengaja tidak punya gestur ini — `pull-refresh-host` ikut dilepas di sana supaya PTR bawaan Android tetap hidup. Yang sengaja dibiarkan kosong: Pengaturan, Brosur, AI Tools, Kelola Hotel, dan sub-halaman jamaah daftar/edit — semuanya berisi isian atau pilihan yang sedang dikerjakan, dan memuat ulang di tengahnya justru merugikan. Penyegar Daftar Jamaah pun tetap menolak jalan saat `hasUnsavedChanges()`. Menambah halaman baru cukup satu baris `usePullRefreshHandler(fn, syaratAktif)`.

Uji: `node --test tests/pull-to-refresh.test.js tests/pull-refresh-registry.test.js tests/jadwal-pull-to-refresh.browser.test.js tests/dashboard-pull-to-refresh.browser.test.js` (browser test memakai WebKit dengan `navigator.standalone` dipalsukan).
...
- Media post/komentar: maksimum 10 item, gambar ≤ 3MB, video ≤ 20MB — batas disinkron di **tiga titik**: klien, `server.js` (`COMMUNITY_MAX_MEDIA_ITEMS` dkk), dan constraint DB. Upload via `POST /api/community/media` ke Bunny CDN path `community/` (fallback Supabase Storage bila Bunny belum dikonfigurasi).
- Video (2026-08-16): composer meng-capture **poster JPEG + dimensi** dari blob lokal saat memilih file (`captureVideoPoster` di `src/lib/videoPoster.ts`, non-blocking — gagal capture tetap terkirim tanpa poster); item media video menyimpan `{url, poster, width, height}` dan dirender `PlyrVideo` dengan atribut `poster` + skeleton pulse (`[data-video-skeleton]`, settle saat poster/frame termuat, timeout 4 dtk) + kotak berukuran stabil pra-load: mode `fit` diberi **lebar px definit** hasil skala dimensi terhadap plafon 30rem — `aspect-ratio` saja pada elemen replaced tanpa ukuran intrinsik TIDAK andal (Chromium meng-nol-kan tinggi, WebKit jatuh ke 300×150), dan `min(100%/…)` di dalam parent `fit-content` dihitung konservatif oleh Chromium. Tanpa semua itu, perangkat hemat data/baterai menampilkan kotak hitam 300×150 karena trik `#t=0.001` tidak men-decode apa pun. Upload `.mov` (QuickTime) **disimpan sebagai `.mp4`** — Bunny menyajikan header per-ekstensi dan menganaktirikan `.mov` (`video/quicktime` + `no-cache` tanpa CORS); byte kontainer qt tetap terputar. Codec non-playable (HEVC/High Efficiency iPhone, ProRes, Dolby Vision) **ditolak saat upload** dengan pesan perbaikan — parser stsd di `lib/community-video.js` (blocklist, bukan allowlist). Purge media ikut menghapus file poster & cek rujukan key `poster`.
...
- Edge: `alhijaz.co` lewat Cloudflare (Flexible SSL) → **nginx `:80`** (`server_name alhijaz.co`, log `/var/log/nginx/access.log`) → node `:3000`; sedangkan `sb.alhijaz.co` (Supabase self-hosted) dan custom domain agent (on-demand TLS) lewat Caddy `:443` → node `:3000` tanpa Cloudflare (log `/var/log/caddy/access.log`). Diverifikasi 21 Sep 2026 — request alhijaz.co TIDAK pernah muncul di log Caddy.
...
### Optimasi kecepatan akses (audit 21 Sep 2026)

Pengukuran (Chromium headless, 4G 9 Mbps/120 ms RTT, CPU 4×, origin lokal) menunjukkan 72% byte muat awal halaman jadwal adalah **font + gambar**, bukan JS: 3× Inter penuh (333 KB) di-preload, bendera PNG 600×401 (412 KB) jadi elemen LCP, `/api/schedules` baru diminta setelah chunk entry (704 KB raw) selesai dieksekusi, dan Cloudflare tidak meng-cache aset (`cf-cache-status: DYNAMIC`). Paket perbaikan yang dipasang:

- **Font subset** — `scripts/subset-fonts.mjs` (devDependency `subset-font`/harfbuzz) menghasilkan `Inter-<Berat>-latin.woff2` (±21 KB) dan `-latin-ext.woff2` per berat 400–900; fitur `tnum/pnum/kern/calt` dipertahankan. `index.html` mendeklarasikan per berat: berkas penuh (`unicode-range: U+0000-FFFF`) → latin-ext → latin (aturan terakhir menang; glyph non-Latin jatuh ke berkas penuh yang hanya diunduh bila dipakai). Preload 4 berkas `-latin` (400/600/700/800). Range latin ditambah simbol UI (★ ☆ ← → ✓) agar berkas penuh tidak ikut terunduh. Tradeoff: teks non-Latin (Cyrillic/Yunani) memicu unduhan berkas penuh — perilaku sama dengan Google Fonts.
- **Bendera WebP** — `scripts/generate-flag-webp.mjs` (sharp, 250 px, ±7 KB) dipakai lewat `<picture>` di watermark PackageCard; PNG tetap ada untuk react-pdf (CompareDocument, itinerary) dan sebagai fallback. Screenshot kartu (snapdom) membuang `<source>` di klon supaya hasilnya identik dengan sebelumnya.
- **Prefetch `/api/schedules`** — skrip inline `index.html` memulai fetch saat HTML diurai (dilewati di rute dashboard/landing/portal, saat cache localStorage < 30 menit, atau agent login membuka `/`), disimpan di `window.__schedulesPrefetch[YEAR]` + stempel `__schedulesPrefetchAt`; `data-service.ts` memakainya **sekali** (dibuang bila > 30 dtk), pull-to-refresh/interval tetap fetch baru; revalidasi saat mount memakai `preferPrefetch`. `YEAR` wajib sama dengan `DEFAULT_YEAR_CODE` (dijaga `tests/schedules-prefetch-year.test.js`).
- **Cache server `/api/schedules`** — `lib/schedule-response-cache.js` (TTL 60 dtk, hanya payload sukses non-degradasi), diinvalidasi di akhir `syncUmrohSchedules()` dan `syncFilesToBunny()`. Tradeoff: data bisa basi maksimal 60 dtk di luar jalur sync (sync sendiri berjalan tiap ±19 menit).
- **`GET /api/agents/public`** — daftar agent (slug, name, website, phone, photo; status active/null) dari cache `getAgents()` menggantikan pembacaan tabel `agents` lewat anon key Supabase di browser; `src/lib/supabase.ts` dihapus sehingga `@supabase/*` (172 KB) keluar dari chunk entry. Daftar kosong (DB gagal, cache dingin) → 503 `no-store`.
- **Diet chunk entry** 704 KB → ±303 KB raw: `DashboardLayout`/`LoginPage`/`RegisterPage`/`ResetPasswordPage` lazy di `main.tsx` (chunk dashboard di-prefetch paralel dengan `/api/auth/me` bila ada sesi), helper sesi dipindah ke `src/lib/authSession.ts` (LoginPage re-export), `PortalJamaahRouter` lazy di `App.tsx`, modal agent di PackageCard (Brochure/Sticker/CaptionAI/PackageValue/BrochurePrompt) lazy dan hanya di-mount setelah pertama dibuka (pola `askAIMounted`). Jangan menambah impor statis modul dashboard ke `main.tsx`/`App.tsx`/`PackageCard.tsx`.
- **Registrasi SW ditunda** sampai `load` + idle (maks 4 dtk) agar unduhan precache (±4 MB) tidak bersaing dengan API/gambar; `activateWaitingWorker` mengirim `SKIP_WAITING` langsung ke `registration.waiting`, dan `src/lib/pwa/installingWorker.ts` menangani SW yang sudah `installing` saat `registerSW` dipanggil (balapan dengan `checkBuildVersion`).
- **Belum dilakukan (butuh dashboard Cloudflare, tidak ada kredensial di server):** Cache Rule agar `/assets/*`, `/fonts/*`, `/flags/*` eligible di edge (Edge TTL ikut origin; kecualikan `/sw.js`, `/registerSW.js`, `/manifest.webmanifest`, `/index.html`). Verifikasi dengan GET dua kali → `cf-cache-status: HIT`. Ini lever terbesar yang tersisa untuk pengunjung alhijaz.co (custom domain tidak lewat CF).
- Opsi lanjutan yang sengaja tidak dipasang: precompress `.br` q11 saat build + `file_server precompressed` (hemat CPU origin & ±10% byte, tapi dua tempat header cache), memangkas precache SW ke shell publik (hemat data, tapi halaman dashboard yang belum pernah dibuka tak tersedia offline), SSL mode Full (Caddy melayani alhijaz.co di :443 — keamanan CF↔origin, bukan kecepatan).
