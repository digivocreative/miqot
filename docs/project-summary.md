...
- Halaman yang TIDAK mendaftar sengaja tidak punya gestur ini — `pull-refresh-host` ikut dilepas di sana supaya PTR bawaan Android tetap hidup. Menambah halaman baru cukup satu baris `usePullRefreshHandler(fn, syaratAktif)`; sub-halaman berisi form (jamaah daftar/edit) sengaja dibiarkan kosong, dan penyegar Daftar Jamaah tetap menolak jalan saat `hasUnsavedChanges()`.
