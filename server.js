...
import { aggregatePackageUsage } from './lib/hotel-master.js';
...
// Gate a feature to a single agent slug. The Hotel feature is exclusive to
// "nikita"; note she is also an admin, so role-based gating would leak it to
// every admin — we must match the slug explicitly.
function nikitaOnly(req, res, next) {
  if (String(req.user?.slug || '').toLowerCase() !== 'nikita') {
    return res.status(403).json({
      error: 'feature_not_available',
      message: 'Fitur Hotel belum tersedia untuk akun ini.',
    });
  }
  next();
}
...
// ──────────────────────────────────────────────
// Hotel directory (exclusive to agent "nikita")
// ──────────────────────────────────────────────
// Curated hotel_master entities + live aggregation of which active packages stay
// at each hotel. All routes are gated by nikitaOnly.

async function loadHotelMasterRows() {
  const { data, error } = await supabase
    .from('hotel_master')
    .select('*')
...
async function loadActiveScheduleRowsForHotels() {
  const { data, error } = await supabase
    .from('umroh_schedules')
    .select('jadwal_id, year_code, jadwal_nama, berangkat_tgl, pulang_tgl, seat_sisa, maskapai, paket_hotel, paket_harga')
    .in('year_code', SCHEDULE_YEAR_CODES);
  if (error) throw error;
  return data || [];
}

function serializeHotelRow(h) {
  return {
    slug: h.slug,
    name: h.name,
    city: h.city,
    ...
    walk_minutes: h.walk_minutes,
    view_haram: !!h.view_haram,
    elderly_friendly: !!h.elderly_friendly,
    facilities: Array.isArray(h.facilities) ? h.facilities : [],
    description: h.description || '',
    aliases: Array.isArray(h.aliases) ? h.aliases : [],
    is_setaraf_class: !!h.is_setaraf_class,
...
// Directory: every hotel + how many active packages use it, seats, cheapest price.
app.get('/api/hotels', authMiddleware, nikitaOnly, async (req, res) => {
  try {
    const [hotels, schedules] = await Promise.all([
      loadHotelMasterRows(),
      loadActiveScheduleRowsForHotels(),
    ]);
    const usage = aggregatePackageUsage(schedules, hotels);
    const items = hotels.map(h => {
      const pkgs = usage.get(h.slug) || [];
      return {
        ...serializeHotelRow(h),
        package_count: pkgs.length,
        seat_available: pkgs.reduce((sum, p) => sum + (p.seat_sisa || 0), 0),
        lowest_price: lowestOf(pkgs.map(p => p.lowest_price)),
        next_departure: pkgs.length ? pkgs[0].berangkat_tgl : null,
      };
    });
    res.json({ hotels: items });
  } catch (err) {
    console.error('[Hotels] directory error:', err.message);
    res.status(500).json({ error: 'Failed to load hotels' });
  }
});

// Detail: one hotel + the active packages that stay there.
app.get('/api/hotels/:slug', authMiddleware, nikitaOnly, async (req, res) => {
  try {
    const slug = String(req.params.slug || '').toLowerCase();
    const [hotels, schedules] = await Promise.all([
      loadHotelMasterRows(),
    ...
    const usage = aggregatePackageUsage(schedules, hotels);
    const packages = usage.get(slug) || [];
    res.json({
      hotel: serializeHotelRow(hotel),
      packages,
      summary: {
        package_count: packages.length,
        seat_available: packages.reduce((sum, p) => sum + (p.seat_sisa || 0), 0),
        lowest_price: lowestOf(packages.map(p => p.lowest_price)),
        next_departure: packages.length ? packages[0].berangkat_tgl : null,
      },
    });
  } catch (err) {
    console.error('[Hotels] detail error:', err.message);
    res.status(500).json({ error: 'Failed to load hotel' });
  }
});

function buildHotelUpdate(body) {
  const updates = {};
  if (body.name !== undefined) updates.name = String(body.name);
  if (body.city !== undefined) updates.city = String(body.city).toLowerCase();
  if (body.stars !== undefined) updates.stars = body.stars === null ? null : Number(body.stars);
  if (body.distance_label !== undefined) updates.distance_label = body.distance_label || null;
  if (body.walk_minutes !== undefined) updates.walk_minutes = body.walk_minutes === null || body.walk_minutes === '' ? null : Number(body.walk_minutes);
  if (body.view_haram !== undefined) updates.view_haram = !!body.view_haram;
  if (body.elderly_friendly !== undefined) updates.elderly_friendly = !!body.elderly_friendly;
  if (body.facilities !== undefined) updates.facilities = Array.isArray(body.facilities) ? body.facilities : [];
  if (body.description !== undefined) updates.description = body.description || null;
  if (body.aliases !== undefined) updates.aliases = Array.isArray(body.aliases) ? body.aliases : [];
  if (body.is_setaraf_class !== undefined) updates.is_setaraf_class = !!body.is_setaraf_class;
...
// Create hotel
app.post('/api/hotels', authMiddleware, nikitaOnly, async (req, res) => {
  const { slug, name, city } = req.body || {};
  if (!slug || !name || !city) {
    return res.status(400).json({ error: 'slug, name, dan city wajib diisi' });
  }
  const insert = { slug: String(slug).toLowerCase(), ...buildHotelUpdate(req.body) };
  insert.name = name;
  insert.city = String(city).toLowerCase();
  const { error } = await supabase.from('hotel_master').insert(insert);
  if (error) return res.status(500).json({ error: error.message });
  res.json({ success: true, slug: insert.slug });
});

// Update hotel
app.put('/api/hotels/:slug', authMiddleware, nikitaOnly, async (req, res) => {
  const slug = String(req.params.slug || '').toLowerCase();
  const updates = buildHotelUpdate(req.body);
  updates.updated_at = new Date().toISOString();
  const { error } = await supabase.from('hotel_master').update(updates).eq('slug', slug);
  if (error) return res.status(500).json({ error: error.message });
  res.json({ success: true });
});

// Delete hotel
app.delete('/api/hotels/:slug', authMiddleware, nikitaOnly, async (req, res) => {
  const slug = String(req.params.slug || '').toLowerCase();
  const { error } = await supabase.from('hotel_master').delete().eq('slug', slug);
  if (error) return res.status(500).json({ error: error.message });
  res.json({ success: true });
});

// Upload a hotel photo (base64) to the public 'hotel-photos' bucket.
app.post('/api/hotels/:slug/photo', authMiddleware, nikitaOnly, express.json({ limit: '8mb' }), async (req, res) => {
  const slug = String(req.params.slug || '').toLowerCase();
  ...
  try {
    ...
    const buffer = Buffer.from(base64Data, 'base64');
    const fileName = `${slug}.jpg`;
    await supabase.storage.from('hotel-photos').remove([fileName]);
    const { error: uploadError } = await supabase.storage
      .from('hotel-photos')
    ...
    const photoUrl = `${urlData.publicUrl}?v=${Date.now()}`;
    const { error: updErr } = await supabase.from('hotel_master').update({ photo_url: photoUrl, updated_at: new Date().toISOString() }).eq('slug', slug);
    if (updErr) throw updErr;
    res.json({ success: true, photo_url: photoUrl });
  } catch (err) {
    console.error('[Hotels] photo upload error:', err.message);
    res.status(500).json({ error: 'Failed to save photo' });
  }
});
