...
function isCommunityMediaSchemaMissing(error) {
  const code = String(error?.code || '');
  const message = String(error?.message || error?.details || '');
  if (!['42703', 'PGRST204'].includes(code)) return false;
  return /\bmedia\b/i.test(message)
    && /community_posts|schema cache|column|field/i.test(message)
    && /does not exist|could not find|schema cache/i.test(message);
}
...
function normalizeStoredCommunityMedia(value, photoUrl = null) {
  ...
  const legacyPhotoUrl = typeof photoUrl === 'string' ? photoUrl.trim() : '';
...
function communityMediaEquals(left, right) {
  if (!Array.isArray(left) || !Array.isArray(right) || left.length !== right.length) return false;
  return left.every((item, index) => (
    item?.type === right[index]?.type && item?.url === right[index]?.url
  ));
}

function hasExpectedCommunityMediaSignature(buffer, mime) {
  if (mime.startsWith('image/')) return hasExpectedImageSignature(buffer, mime);
  if (mime === 'video/webm') {
    return buffer.length >= 4
      ...
      && buffer[2] === 0xdf
      && buffer[3] === 0xa3
      && buffer.subarray(0, Math.min(buffer.length, 256)).includes(Buffer.from('webm'));
  }
  if (!['video/mp4', 'video/quicktime'].includes(mime) || buffer.length < 12) return false;
  if (buffer.toString('ascii', 4, 8) !== 'ftyp') return false;
  const majorBrand = buffer.toString('ascii', 8, 12);
  if (mime === 'video/quicktime') return majorBrand === 'qt  ';
  return /^(?:isom|iso[2-9]|mp4[12]|avc1|dash|M4V |MSNV|3g[p2][4-9])$/.test(majorBrand);
}

function isCommunityStorageConflict(error) {
  return Number(error?.statusCode || error?.status) === 409
    || /already exists|duplicate/i.test(String(error?.message || error?.error || ''));
}

async function prepareCommunityMediaUpload(req, res, next) {
  try {
    const agent = await getAgentById(req.user.id);
    if (!agent) return res.status(404).json({ error: 'Agent not found' });
    ...
    const now = Date.now();
    const reservedBytes = contentLength ?? mediaConfig.maxBytes;
    let windowState = communityMediaUploadWindows.get(agent.id);
    if (!windowState || windowState.resetAt <= now) {
      windowState = { count: 0, bytes: 0, resetAt: now + COMMUNITY_MEDIA_RATE_WINDOW_MS };
    }
    if (
      windowState.count >= COMMUNITY_MEDIA_RATE_MAX_UPLOADS
      || windowState.bytes + reservedBytes > COMMUNITY_MEDIA_RATE_MAX_BYTES
    ) {
      res.set('Retry-After', String(Math.max(1, Math.ceil((windowState.resetAt - now) / 1000))));
      return res.status(429).json({ error: 'Terlalu banyak unggahan media. Coba lagi beberapa menit.' });
    }
    windowState.count += 1;
    windowState.bytes += reservedBytes;
    communityMediaUploadWindows.set(agent.id, windowState);

    req.communityAgent = agent;
    req.communityMediaConfig = mediaConfig;
    req.communityMediaMime = mime;
    req.communityUploadId = uploadId;
    return next();
  } catch (err) {
    console.error('[community] media authorization error:', err);
    return res.status(500).json({ error: 'Gagal memvalidasi unggahan media' });
  }
}

function parseCommunityMediaBody(req, res, next) {
...
async function loadTerasNotificationSources(agent, { since, clearedAt = null, limit, prefs }) {
  ...
  const broadcastQuery = !flags.broadcasts ? null : runCommunityRootQuery(withThread => {
    let query = supabase
      .from('community_posts')
...
app.post('/api/community/media', authMiddleware, prepareCommunityMediaUpload, parseCommunityMediaBody, async (req, res) => {
  try {
    const agent = req.communityAgent;
    const uploadId = req.communityUploadId;
    const mime = req.communityMediaMime;
    const mediaConfig = req.communityMediaConfig;
...
async function generateCalendarInsight() {
  ...
  try {
    const [weekResult, monthResult] = await Promise.all([
      ...
      supabase.from('calendar_events').select('event_date, event_type, pax, pax_jamaah, pax_terisi, paket, group_number').gte('event_date', monthStart).lt('event_date', monthEnd).eq('event_type', 'keberangkatan'),
    ]);
  ...
  for (const ev of validEvents) {
    ...
    weekSummary[key].groups.push({
      ...
      // Kursi terisi nasional bila ter-map; fallback kuota legacy (konsisten dgn kalender)
      pax: ev.pax_terisi ?? ev.pax ?? 0,
      paket: ev.paket,
    ...
    weekSummary[key].totalPax += ev.pax_terisi ?? ev.pax ?? 0;
  }
  ...
  // Month summary
  const monthTotalPax = monthEvents.reduce((s, e) => s + (e.pax_terisi ?? e.pax ?? 0), 0);
...
const KNOWN_ROUTES = {
  ...
  'EK357':  { dep: 'CGK', depCity: 'Jakarta',  arr: 'DXB', arrCity: 'Dubai',    durationMin: 470, depTerminal: '3' },
  'EK803':  { dep: 'DXB', depCity: 'Dubai',    arr: 'JED', arrCity: 'Jeddah',   durationMin: 180 },
  'EK809':  { dep: 'DXB', depCity: 'Dubai',    arr: 'MED', arrCity: 'Madinah',  durationMin: 175 },
  'EK802':  { dep: 'JED', depCity: 'Jeddah',   arr: 'DXB', arrCity: 'Dubai',    durationMin: 170 },
  'EK808':  { dep: 'MED', depCity: 'Madinah',  arr: 'DXB', arrCity: 'Dubai',    durationMin: 170 },
  'EK358':  { dep: 'DXB', depCity: 'Dubai',    arr: 'CGK', arrCity: 'Jakarta',  durationMin: 515 },
...
function lookupKnownRoute(flightIata) {
  return KNOWN_ROUTES[flightIata] || null;
}

function lookupRoute(flightIata, eventType) {
  const route = lookupKnownRoute(flightIata);
...
function routeForFlightSegment(segment, event, schedule, segmentIndex) {
  ...
  const routeLegs = parseRouteLegs(routeStringForEventType(schedule, event.event_type));
  const leg = routeLegs[segmentIndex];
  if (!leg) return known || fallback;

  const pairKey = `${leg.dep}-${leg.arr}`;
  const knownMatchesLeg = known?.dep === leg.dep && known?.arr === leg.arr;
  const durationMin = (knownMatchesLeg ? known.durationMin : null)
    || ROUTE_PAIR_DURATIONS[pairKey]
    || known?.durationMin
  ...
  return {
    ...
    ...(known || {}),
    dep: leg.dep,
    depCity: airportCity(leg.dep),
    arr: leg.arr,
    arrCity: airportCity(leg.arr),
    durationMin,
    depTerminal: knownMatchesLeg ? known.depTerminal || null : null,
  };
}
 ...
 * Legacy convention: `jam` is the Indonesia-side time for the whole trip:
 *   keberangkatan → first segment take-off,
 *   kepulangan    → final segment landing.
 * Multi-leg transit cards are estimated as a continuous chain because the
...
function deriveCalendarFlightSegmentTimes(event, routes) {
  ...
  const fallbackRoute = lookupRoute('', event.event_type) || {
    dep: isKepulangan ? 'JED' : 'CGK',
    arr: isKepulangan ? 'CGK' : 'JED',
    durationMin: 540,
  };
  const chain = (routes?.length ? routes : [fallbackRoute]).map(route => route || fallbackRoute);
  ...
  const buildTimes = (route, depUTC, arrUTC, operationalDateTrusted) => {
    ...
    return {
      ...
      depDateLocal: toAirportDate(depUTC, depTZ),
      arrDateLocal: toAirportDate(arrUTC, arrTZ),
      depUTC,
      arrUTC,
  ...
  const out = new Array(chain.length);
  if (isKepulangan) {
    const lastRoute = chain[chain.length - 1];
    const finalArrTZ = AIRPORT_TZ_OFFSETS[lastRoute.arr] || 7;
    let arrUTC = jamMs - finalArrTZ * 60 * 60 * 1000;
    ...
    const firstRoute = chain[0];
    const firstDepTZ = AIRPORT_TZ_OFFSETS[firstRoute.dep] || 7;
    let depUTC = jamMs - firstDepTZ * 60 * 60 * 1000;
...
/**
 * Single-leg compatibility wrapper.
...
function deriveCalendarFlightTimes(event, route) {
  return deriveCalendarFlightSegmentTimes(event, [route])[0] || null;
}
...
function parseFlightFromCalendar(pesawat) {
  return parseFlightSegmentsFromCalendar(pesawat)[0] || null;
}
 ...
 */
function mapAirLabsToFlightStatus(apiData, calendarEvent, flightSegment = null) {
  ...
  const result = {
    ...
    pax: calendarEvent.pax_terisi ?? calendarEvent.pax ?? 0,
  ...
  const route = flightSegment?.route || lookupRoute(parsed.flightIata, calendarEvent.event_type);
  if (route) {
...
async function loadFlightScheduleMap(events) {
  const jadwalIds = [...new Set((events || []).map(e => e.jadwal_id).filter(Boolean).map(String))];
  if (jadwalIds.length === 0) return new Map();

  const { data, error } = await supabase
    .from('umroh_schedules')
    .select(FLIGHT_SCHEDULE_SELECT)
    .in('jadwal_id', jadwalIds)
    .order('year_code', { ascending: false });

  if (error) {
    console.warn('[Flights] Schedule lookup failed:', error.message);
    return new Map();
  }

  const map = new Map();
  for (const row of data || []) {
    const key = String(row.jadwal_id);
      ...
      for (const segment of segments) {
        ...
        if (flightBase) {
          const entry = {
            ...
            direction: event.event_type === 'kepulangan' ? 'pulang' : 'pergi',
...
function getHoursUntilDeparture(event, flightSegment = null) {
  const times = flightSegment?.times || deriveCalendarFlightTimes(
    event,
    flightSegment?.route || lookupRoute(flightSegment?.flightIata || '', event.event_type)
  );
...
function buildFlightNotifMessage(flight, calendarEvent, change) {
  ...
  const paxLine = `Jamaah: <b>${(calendarEvent.pax_terisi ?? calendarEvent.pax) || '?'} orang</b>`;
...
async function pollActiveFlights() {
  ...
  for (const event of events) {
    ...
    for (const segment of segments) {
      ...
      const hoursUntilDep = getHoursUntilDeparture(event, segment);
      ...
      const apiData = await fetchFlightFromAirLabs(segment.flightIata);
      pollCount++;

      if (!apiData) continue;

      const mapped = mapAirLabsToFlightStatus(apiData, event, segment);
      if (!mapped) continue;
